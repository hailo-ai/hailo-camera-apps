...
TEST_CASE("camera-viewer-server test frontend framerate", "[camera-viewer-server]") {
    run_server();
    std::atomic<bool> stop_flag(false);
    std::atomic<bool> error_detected(false);
    std::thread log_thread(monitor_logs, LOG_FILE, std::ref(stop_flag), std::ref(error_detected));
    try{
        ...
        body_frontend["application_input_streams"]["resolutions"][0]["framerate"] = 20;
        nlohmann::json rotation_payload = nlohmann::json();
        rotation_payload["application_input_streams"] = body_frontend["application_input_streams"];
...
TEST_CASE("camera-viewer-server test frondend rotation", "[camera-viewer-server]") {
    run_server();
    std::atomic<bool> stop_flag(false);
    std::atomic<bool> error_detected(false);
    std::thread log_thread(monitor_logs, LOG_FILE, std::ref(stop_flag), std::ref(error_detected));
...
TEST_CASE("camera-viewer-server test frondend resolution", "[camera-viewer-server]") {
    run_server();
    std::atomic<bool> stop_flag(false);
    std::atomic<bool> error_detected(false);
    std::thread log_thread(monitor_logs, LOG_FILE, std::ref(stop_flag), std::ref(error_detected));
    try{
        ...
        //change resolution
        nlohmann::json body_frontend = load_payload("frontend.json");
        body_frontend["application_input_streams"]["resolutions"][0]["width"] = 1280;
        body_frontend["application_input_streams"]["resolutions"][0]["height"] = 720;
        nlohmann::json resolution_payload = nlohmann::json();
        resolution_payload["application_input_streams"] = body_frontend["application_input_streams"];
