...
{
    return true ? TRIPLE_A_CONFIG_PATH_SIM_LIMK : TRIPLE_A_CONFIG_PATH;
}

void webserver::common::update_3a_config(bool auto_algo, const std::string default_3a_path)
{
    std::lock_guard<std::recursive_mutex> lock(g_file_config_mutex);
    WEBSERVER_LOG_INFO("Updating 3A config to auto_algo: {}", auto_algo);
    // Read JSON file
    nlohmann::json config = get_3a_config();
    nlohmann::json default_config;
    if (!default_3a_path.empty())
    {
        default_config = get_3a_config(default_3a_path);
    }

    for (std::string classname : {ISP_CLASSNAME_AEEV1, ISP_CLASSNAME_ACPROC, ISP_CLASSNAME_AUTO_WHITE_BALANCE})
    {
        bool class_enabled = auto_algo;
        bool class_disabled = false;

        auto obj_opt = get_3a_config_class(config, classname);
        if (!obj_opt.has_value())
        {
            WEBSERVER_LOG_ERROR("Class {} not found in 3A config", classname);
            continue;
        }
        auto &obj = obj_opt.value().get();

        if (!default_3a_path.empty() && auto_algo)
        {
            auto default_obj_opt = get_3a_config_class(default_config, classname);
            if (!default_obj_opt.has_value())
            {
                WEBSERVER_LOG_ERROR("Default class {} not found in 3A config", classname);
                continue;
            }
            auto &default_obj = default_obj_opt.value().get();
            class_enabled = default_obj["enable"].get<bool>();
            class_disabled = default_obj["disable"].get<bool>();
        }

        obj["enable"] = class_enabled;
        obj["disable"] = class_disabled;
    }

    // Write updated JSON back to file
    update_3a_config(config);
}

std::optional<std::reference_wrapper<nlohmann::json>> webserver::common::get_3a_config_class(
    nlohmann::json &config, const std::string &classname)
{
    // If classname is not provided, return the whole config
    if (classname.empty())
    {
        return std::ref(config);
    }

    // Check if classname exists in root
    if (config.contains("root") && !config["root"].is_array())
    {
        throw std::runtime_error("Root is not an array in 3A config");
    }
    ...
    if (config.contains("root"))
    {
        ...
        {
            if (obj.contains("classname") && obj["classname"] == classname)
            {
                return std::ref(obj);
            }
        }
    }
    else if (config.contains("automatic_algorithms"))
    {
        if (!config["automatic_algorithms"].contains(classname))
        {
            WEBSERVER_LOG_ERROR("Class {} not found in automatic_algorithms", classname);
            return std::nullopt;
        }
        auto &obj = config["automatic_algorithms"][classname];
        return std::ref(obj);
    }
    else
    {
        WEBSERVER_LOG_ERROR("No root or automatic_algorithms found in 3A config");
    }
    return std::nullopt; // Return nullopt if classname not found
}
...
void webserver::common::update_3a_config(nlohmann::json config)
{
    ...
    outFile << config.dump(4); // 4 spaces indentation for pretty print
    outFile.close();
}

nlohmann::json webserver::common::get_3a_config(const std::string path)
{
    std::lock_guard<std::recursive_mutex> lock(g_file_config_mutex);
    nlohmann::json config;
    // Read JSON file
    std::ifstream file(path.empty() ? get_triple_a_config_path() : path);
    if (!file.is_open())
    {
