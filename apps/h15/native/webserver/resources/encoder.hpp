...
class EncoderResource : public Resource
{
  public:
    ...
    struct rate_control_t
    {
        ...
        void set_rate_control_on(bitrate_control_t rc_mode, int bitrate, uint32_t qp_min, uint32_t qp_max,
                                 int32_t intra_qp_delta, uint32_t fixed_intra_qp, int32_t qp_hdr)
        {
    ...
    struct encoder_control_t
    {
        ...
        const std::unordered_map<codec_mode_t, codec_t> codec_to_hailo_codec{{H264, CODEC_TYPE_H264},
                                                                             {H265, CODEC_TYPE_HEVC}};
        const std::unordered_map<codec_t, codec_mode_t> hailo_codec_to_codec{{CODEC_TYPE_H264, H264},
                                                                             {CODEC_TYPE_HEVC, H265}};
        const std::unordered_map<codec_mode_t, std::string> codec_to_str{{H264, "CODEC_TYPE_H264"},
                                                                         {H265, "CODEC_TYPE_HEVC"}};
        const std::unordered_map<std::string, codec_mode_t> str_to_codec{{"CODEC_TYPE_H264", H264},
                                                                         {"CODEC_TYPE_HEVC", H265}};

        const std::unordered_map<bitrate_control_t, rc_mode_t> rc_mode_to_hailo_rc_mode{
            {VBR, rc_mode_t::VBR}, {CVBR, rc_mode_t::CVBR}, {CQP, rc_mode_t::CQP}};
        const std::unordered_map<rc_mode_t, bitrate_control_t> hailo_rc_mode_to_rc_mode{
            {rc_mode_t::VBR, VBR}, {rc_mode_t::CVBR, CVBR}, {rc_mode_t::CQP, CQP}};
        const std::unordered_map<bitrate_control_t, std::string> rc_mode_to_str{
            {VBR, "VBR"}, {CVBR, "CVBR"}, {CQP, "CQP"}};
        const std::unordered_map<std::string, bitrate_control_t> str_to_rc_mode{
            {"VBR", VBR}, {"CVBR", CVBR}, {"CQP", CQP}};
    };
