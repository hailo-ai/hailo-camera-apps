...
void webserver::resources::to_json(nlohmann::json &j, const webserver::resources::EncoderResource::encoder_control_t &b)
{
    ...
    j = nlohmann::json{
        ...
        {"b_frames", {{"num_b_frames", b.b_frames.num_b_frames}, {"b_frame_qp_delta", b.b_frames.b_frame_qp_delta}}}};
    if (!(b.quantization.rc_mode == webserver::resources::EncoderResource::bitrate_control_t::CQP))
    {
        if (!b.quantization.bitrate.has_value() || !b.quantization.qp_min.has_value() ||
            !b.quantization.qp_max.has_value() || !b.quantization.picture_rc)
            WEBSERVER_LOG_ERROR(
                "Encoder config does not have bitrate, qp_min, qp_max or picture_rc while rc_mode is not CQP");
        j["quantization"].merge_patch(nlohmann::json{{"bitrate", b.quantization.bitrate.value()},
                                                     {"qp_min", b.quantization.qp_min.value()},
                                                     {"qp_max", b.quantization.qp_max.value()}});
    }
    else if (b.quantization.bitrate.has_value() || b.quantization.qp_min.has_value() ||
             b.quantization.qp_max.has_value() || b.quantization.picture_rc)
        WEBSERVER_LOG_ERROR("Encoder config have bitrate, qp_min, qp_max or picture_rc while rc_mode is not CQP");
}

void webserver::resources::from_json(const nlohmann::json &j,
                                     webserver::resources::EncoderResource::encoder_control_t &b)
{
    ...
    if (rc_mode == webserver::resources::EncoderResource::bitrate_control_t::CQP)
        ...
        quantization.set_rate_control_on(rc_mode, j.at("quantization").at("bitrate").get<uint32_t>(),
...
void webserver::resources::EncoderResource::reset_config()
{
    ...
    m_encoder_control.quantization.rc_mode =
        m_encoder_control.str_to_rc_mode.at(m_config["hailo_encoder"]["rate_control"]["rc_mode"]);
    ...
    m_encoder_control.codec =
        m_encoder_control.str_to_codec.at(m_config["hailo_encoder"]["config"]["output_stream"]["codec"]);
...
void webserver::resources::EncoderResource::set_encoder_control(
    webserver::resources::EncoderResource::encoder_control_t &encoder_control)
{
    ...
    if ((encoder_control.quantization.rc_mode == webserver::resources::EncoderResource::bitrate_control_t::CQP &&
         (encoder_control.quantization.bitrate.has_value() || encoder_control.quantization.qp_min.has_value() ||
          encoder_control.quantization.qp_max.has_value() || encoder_control.quantization.picture_rc)) ||
    ...
    m_config["hailo_encoder"]["config"]["output_stream"]["codec"] =
        encoder_control.codec_to_str.at(encoder_control.codec);
    m_config["hailo_encoder"]["rate_control"]["rc_mode"] =
        encoder_control.rc_mode_to_str.at(encoder_control.quantization.rc_mode);

    if (encoder_control.quantization.bitrate.has_value())
        m_config["hailo_encoder"]["rate_control"]["bitrate"]["target_bitrate"] =
            encoder_control.quantization.bitrate.value();
    ...
    if (old_codec != encoder_control.codec)
    {
        ...
        on_resource_change(EventType::CODEC_CHANGE,
                           std::make_shared<webserver::resources::EncoderResource::EncoderResourceState>(
                               EncoderResourceState(m_encoder_control)));
    }
}
