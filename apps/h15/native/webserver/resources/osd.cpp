    ...
    subscribe_callback({EventType::CHANGE_ROTATION, EventType::CHANGE_RESOLUTION}, EventPriority::EVENT_PRIORITY_LOW,
                       [this](ResourceStateChangeNotification notification) {
                           ...
                           on_resource_change(EventType::CHANGED_RESOURCE_OSD,
                                              std::make_shared<OsdResourceState>(m_config, std::vector<std::string>{}));
                       });
    subscribe_callback(EventType::SWITCH_PROFILE, EventPriority::EVENT_PRIORITY_LOW,
                       [this, configs](ResourceStateChangeNotification notification) {
                           WEBSERVER_LOG_INFO("Received SWITCH_PROFILE notification");
                           m_default_config = configs->get_osd_and_encoder_default_config().dump(4);
                           on_resource_change(EventType::CHANGED_RESOURCE_OSD,
                                              std::make_shared<OsdResourceState>(m_config, get_all_overlays_ids()));
                       });
}
...
void OsdResource::reset_config()
{
    ...
    on_resource_change(EventType::CHANGED_RESOURCE_OSD,
                       std::make_shared<OsdResourceState>(m_config, overlays_to_delete));
...
OsdResource::OsdResourceState::OsdResourceState(nlohmann::json config, std::vector<std::string> overlays_ids)
    : overlays_to_delete(overlays_ids)
{
...
void OsdResource::http_register(std::shared_ptr<HTTPServer> srv)
{
    ...
    srv->Patch("/osd", [this](const nlohmann::json &partial_config) {
        ...
        auto state = std::make_shared<OsdResource::OsdResourceState>(
            OsdResourceState(m_config, get_overlays_to_delete(previouse_config, m_config)));
