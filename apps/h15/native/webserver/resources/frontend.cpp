...
void webserver::resources::FrontendResource::http_register(std::shared_ptr<HTTPServer> srv)
{
    ...
    srv->Patch("/frontend", [this](const nlohmann::json &partial_config)
        ...
        on_resource_change(std::make_shared<webserver::resources::ConfigResourceState>(state));
        return result; });
    ...
    srv->Put("/frontend", [this](const nlohmann::json &config)
        ...
        auto result = this->m_config;
        on_resource_change(std::make_shared<webserver::resources::ConfigResourceState>(ConfigResourceState(this->to_string())));
        return result; });

    srv->Post("/frontend/freeze", std::function<void(const nlohmann::json &)>([this](const nlohmann::json &j_body)
                                                                              {
        bool value = j_body["value"];
        on_resource_change(std::make_shared<webserver::resources::ValueResourceState<bool>>(ValueResourceState<bool>(VALUE_RESOURCE_NAME_FRONTEND_FREEZE, value))); }));
}
