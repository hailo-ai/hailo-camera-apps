...
std::string GStreamerPipeline::create_gst_pipeline_string()
{
    ...
    auto hailort_device_id = fe_config["hailort"]["device-id"];
    ...
    pipeline << "hailonet name=detection batch-size=4 hef-path=" << DETECTION_HEF_PATH << " pass-through=" << detection_pass_through << " nms-iou-threshold=0.45 nms-score-threshold=0.3 scheduling-algorithm=1 scheduler-threshold=4 scheduler-timeout-ms=1000 vdevice-group-id=" << hailort_device_id << " ! ";
