...
osd::TextOverlay text_overlay("text_overlay", 0.4, 0.4, "Chaos App", red_argb, blue_argb, 60.0f, 1, 1, font_path, 0,
                              osd::rotation_alignment_policy_t::CENTER);
osd::ImageOverlay image_overlay("image_overlay", 0.5, 0.5, 0.1, 0.1, image_path, 1, 0,
                                osd::rotation_alignment_policy_t::CENTER);
osd::DateTimeOverlay date_time_overlay("date_time_overlay", 0.2, 0.8, blue_argb, 60.0f, 1, 1, 0,
                                       osd::rotation_alignment_policy_t::CENTER);
...
void OSD_scenario(int &counter, MediaLibraryEncoderPtr encoder, std::string id)
{
    ...
    {
    case 1: {
        encoder->get_osd_blender()->add_overlay(custom_overlay);
        auto custom_expected = encoder->get_osd_blender()->get_overlay("custom_argb1");
        ...
        encoder->get_osd_blender()->set_overlay_enabled("custom_argb1", true);
        std::cout << "adding custom" << std::endl;
        break;
    }
    case 2:
        encoder->get_osd_blender()->add_overlay(text_overlay);
        encoder->get_osd_blender()->set_overlay_enabled("text_overlay", true);
        std::cout << "adding text" << std::endl;
        break;
    case 3:
        encoder->get_osd_blender()->add_overlay(date_time_overlay);
        encoder->get_osd_blender()->set_overlay_enabled("date_time_overlay", true);
        std::cout << "adding date time" << std::endl;
        break;
    case 4:
        encoder->get_osd_blender()->add_overlay(image_overlay);
        encoder->get_osd_blender()->set_overlay_enabled("image_overlay", true);
        std::cout << "adding image" << std::endl;
        break;
    case 5:
        encoder->get_osd_blender()->set_overlay_enabled("custom_argb1", false);
        std::cout << "removing custom" << std::endl;
        break;
    case 6:
        encoder->get_osd_blender()->set_overlay_enabled("text_overlay", false);
        std::cout << "removing text" << std::endl;
        break;
    case 7:
        encoder->get_osd_blender()->set_overlay_enabled("date_time_overlay", false);
...
*/
    ...
    */
    ...
    case 426:
        ...
        break;
    case 1871:
        //    encoder->get_osd_blender()->remove_overlay("custom_argb1");
        encoder->get_osd_blender()->remove_overlay("text_overlay");
        encoder->get_osd_blender()->remove_overlay("date_time_overlay");
        encoder->get_osd_blender()->remove_overlay("image_overlay");
    ...
    default:
        ...
        */
        ...
        /// TODO: MSW-6108 - When moving/spinning image overlay + set vision config - the app freezes and doesn't
        /// release
...
void update_encoder_param(MediaLibraryEncoderPtr encoder, std::string param_to_change, uint32_t new_value)
{
    ...
    if (encoding_format == EncoderType::Hailo)
    {
        ...
        {
            std::cout << " current" << param_to_change << h264_encoder_config.rate_control.quantization.qp_max.value()
                      << " Setting to " << new_value << std::endl;
            h264_encoder_config.rate_control.quantization.qp_max = new_value;
        }
        else if (param_to_change == "bitrate")
        {
            std::cout << " current" << param_to_change << h264_encoder_config.rate_control.bitrate.target_bitrate
                      << " Setting to " << new_value << std::endl;
    ...
    else if (encoding_format == EncoderType::Jpeg)
    {
        ...
        {
            std::cout << " current" << param_to_change << mjpeg_encoder_config.quality << " Setting to " << new_value
                      << std::endl;
...
void add_static_privacy_masks(PrivacyMaskBlenderPtr privacy_mask_blender)
{
    for (int i = 0; i < 8; ++i)
    {
        ...
        example_polygon.vertices.push_back(vertex(i * privacy_mask_build_factor + i * privacy_mask_build_factor,
                                                  i * privacy_mask_build_factor + i * privacy_mask_build_factor));
        example_polygon.vertices.push_back(
            vertex(i * privacy_mask_build_factor + 60 + i * privacy_mask_build_factor,
                   i * privacy_mask_build_factor + 20 + i * privacy_mask_build_factor + i * 10));
        example_polygon.vertices.push_back(
            vertex(i * privacy_mask_build_factor + 120 + i * privacy_mask_build_factor + i * 10,
                   i * privacy_mask_build_factor + 20 + i * privacy_mask_build_factor));
        example_polygon.vertices.push_back(
            vertex(i * privacy_mask_build_factor + 180 + i * privacy_mask_build_factor,
                   i * privacy_mask_build_factor + 60 + i * privacy_mask_build_factor + i * 10));
        example_polygon.vertices.push_back(vertex(i * privacy_mask_build_factor + 180 + i * privacy_mask_build_factor,
                                                  i * privacy_mask_build_factor + 120 + i * privacy_mask_build_factor));
        example_polygon.vertices.push_back(
            vertex(i * privacy_mask_build_factor + 120 + i * privacy_mask_build_factor,
                   i * privacy_mask_build_factor + 180 + i * privacy_mask_build_factor - i * 10));
        example_polygon.vertices.push_back(
            vertex(i * privacy_mask_build_factor + 60 + i * privacy_mask_build_factor - i * 10,
                   i * privacy_mask_build_factor + 180 + i * privacy_mask_build_factor));
        example_polygon.vertices.push_back(vertex(i * privacy_mask_build_factor + i * privacy_mask_build_factor,
                                                  i * privacy_mask_build_factor + 120 + i * privacy_mask_build_factor));
...
void remove_all_polygons(PrivacyMaskBlenderPtr privacy_mask_blender)
{
    ...
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask2");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask3");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask4");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask5");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask6");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask7");
    privacy_mask_blender->remove_static_privacy_mask("privacy_mask8");
}

void privacy_mask_scenario(int &counter, PrivacyMaskBlenderPtr privacy_mask_blender)
{
    tl::expected<polygon, media_library_return> polygon_exp1;
    polygon polygon1;
    tl::expected<polygon, media_library_return> polygon_exp2;
    polygon polygon2;
    tl::expected<polygon, media_library_return> polygon_exp3;
    polygon polygon3;
    tl::expected<polygon, media_library_return> polygon_exp4;
    polygon polygon4;
    switch (counter)
    {
    case 1:
        add_static_privacy_masks(privacy_mask_blender);
    ...
    case 3:
        ...
        break;
    case 4:
        polygon_exp2 = privacy_mask_blender->get_static_privacy_mask("privacy_mask2");
        polygon2 = polygon_exp2.value();
        polygon2.vertices.erase(polygon2.vertices.begin());
        privacy_mask_blender->set_static_privacy_mask(polygon2);
    ...
    case 5:
        ...
        break;
    case 6:
        polygon_exp1 = privacy_mask_blender->get_static_privacy_mask("privacy_mask1");
        ...
        break;
    case 7:
        rgb_color_t color1_privacy_mask;
    ...
    default:
        ...
        {
            polygon_exp4 = privacy_mask_blender->get_static_privacy_mask("privacy_mask4");
            polygon4 = polygon_exp4.value();
            for (int i = 0; i < 8; i++)
            {
                polygon4.vertices[i].x = polygon4.vertices[i].x - 10;
                privacy_mask_blender->set_static_privacy_mask(polygon4);
            }
        }
        if (counter >= 500 && counter <= 1000)
        {
            polygon_exp3 = privacy_mask_blender->get_static_privacy_mask("privacy_mask3");
            polygon3 = polygon_exp3.value();
            for (int i = 0; i < 8; i++)
            {
                polygon3.vertices[i].y = polygon3.vertices[i].y + 10;
                privacy_mask_blender->set_static_privacy_mask(polygon3);
            }
        }
...
void encoder_scenario(int &counter, MediaLibraryEncoderPtr encoder, bool &encoder_is_running, std::string config_string)
{
    ...
    if (encoding_format == EncoderType::Hailo)
    {
        ...
        /// TODO: MSW-6372 - When dynamically setting qp_max to 10 and doing 3 times set_config (without any change) the
        /// app freezes
        /*
        if (counter % 75 == 0) {
            if (counter % 150 == 0)
    ...
    */
    ...
    /// TODO: MSW-7390 - When running a few streams and toggling start stop of their encoders- errors received and app
    /// crashes
    /*
...
void vision_scenario(int &counter, MediaLibraryFrontendPtr frontend)
{
    ...
    if (counter % 23 == 0)
    {
        ...
        config.multi_resize_config.digital_zoom_config.enabled =
            !config.multi_resize_config.digital_zoom_config.enabled;
        change = true;
    }
    if (counter % 29 == 0)
    {
        std::cout << "grayscale enabled = " << !config.multi_resize_config.application_input_streams_config.grayscale
                  << std::endl;
        config.multi_resize_config.application_input_streams_config.grayscale =
            !config.multi_resize_config.application_input_streams_config.grayscale;
        change = true;
    }
    if (counter % 31 == 0)
