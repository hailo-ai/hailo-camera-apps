...
{
    //Get media config path - get the path from subclass override by default, if config override is available then use it instead
...
tl::expected<bool, CamAppReturnCode> CameraAppConstructor::initialize()
{
    auto config = get_media_config_path();
    std::string config_path = config.value();
    ...
    std::string medialib_config_string((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
    file.close();
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("media library init failed at {}", __FUNCTION__);
        return tl::unexpected(CamAppReturnCode::MEDIA_LIBRARY_INIT_FAILED);
    }

    //Set the start profile of the config if config override is found, otherwise use the default from media config file
    if (m_config_override && !m_config_override->m_start_profile_name.empty())
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("app extension registration failed at {}", __FUNCTION__);
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Failed to get current profile at {}", __FUNCTION__);
        return tl::unexpected(CamAppReturnCode::FAILED);
    }
    else 
    {
    ...
    if (profile.sensor_config.input_video.source_type == frontend_src_element_t::APPSRC)
    {
        ...
        auto frontend_from_file = FrontendStageFromFileBuild::create()
                                            ...
                                            .set_buffer_pool_size(120)
                                            .buildptr();
        m_components.m_frontend_stage = std::static_pointer_cast<FrontendStage>(frontend_from_file);
        AppStatus frontend_config_status = frontend_from_file->configure(m_media_library->m_frontend);
        if (frontend_config_status != AppStatus::SUCCESS)
        {
            REFERENCE_CAMERA_LOG_ERROR("Failed to configure frontend from file at {}", __FUNCTION__);
        ...
        {
            REFERENCE_CAMERA_LOG_ERROR("Failed to set override parameters at {}", __FUNCTION__);
            return tl::unexpected(CamAppReturnCode::FRONTEND_STAGE_CONFIG_FAILED);
        }
    }
    else // frontend source from camera
    {
        ...
        m_components.m_frontend_stage = FrontendStageBuild::create().set_stage_name(StageNames::frontend).buildptr();
        AppStatus frontend_config_status = m_components.m_frontend_stage->configure(m_media_library->m_frontend);
        if (frontend_config_status != AppStatus::SUCCESS)
        {
            REFERENCE_CAMERA_LOG_ERROR("Failed to configure frontend at {}", __FUNCTION__);
            return tl::unexpected(CamAppReturnCode::FRONTEND_STAGE_CONFIG_FAILED);
        }    
    }
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Failed to get stream ids at {}", __FUNCTION__);
        return tl::unexpected(CamAppReturnCode::FRONTEND_FAILED_TO_GET_STREAM_ID);
    }

    // Create and configure endcoders
    for (const auto &entry : m_media_library->m_encoders)
    {
        ...
        std::shared_ptr<EncoderStage> encoder_stage = EncoderStageBuild::create().set_stage_name(stream_id).buildptr();
        m_components.m_encoder_stages[stream_id].encoder_stage_ptr = encoder_stage;

        AppStatus enc_config_status = encoder_stage->configure(m_media_library->m_encoders[stream_id]);
        if (enc_config_status != AppStatus::SUCCESS)
        {
            REFERENCE_CAMERA_LOG_ERROR("Failed to configure encoder at {}", __FUNCTION__);
        ...
        {
            REFERENCE_CAMERA_LOG_ERROR("Unsupported encoder config at {}", __FUNCTION__);
            return tl::unexpected(CamAppReturnCode::ENCODER_STAGE_CONFIG_FAILED);
        }
