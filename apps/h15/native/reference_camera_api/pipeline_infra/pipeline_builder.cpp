...
void PipelineBuilder::next_state(building_state state)
{
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Invalid state transition from {} to {}", static_cast<int>(m_state),
                                   static_cast<int>(state));
        throw std::invalid_argument("Invalid state transition");
    }
    REFERENCE_CAMERA_LOG_DEBUG("Transitioning state from {} to {}", static_cast<int>(m_state), static_cast<int>(state));
    m_state = state;
}
...
PipelineBuilder &PipelineBuilder::connect(const std::string &sourceName, const std::string &targetName)
{
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Invalid state in connect. Expected CONNECTING but current state is {}",
                                                   ...
                                                   const std::string &targetName)
{
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Invalid state in connect_frontend. Expected CONNECTING but current state is {}",
                                   static_cast<int>(m_state));
        throw std::invalid_argument("PipelineBuilder is not in CONNECTING state");
    }
    ...
    if (m_frontendStages.find(targetName) != m_frontendStages.end())
    {
        REFERENCE_CAMERA_LOG_ERROR("connect_frontend target cannot be a FrontendStage: {}", targetName);
        throw std::invalid_argument("connect_frontend target cannot be a FrontendStage: " + targetName);
    }
    if (m_genericStages.find(targetName) == m_genericStages.end())
    {
        REFERENCE_CAMERA_LOG_ERROR("Target stage not found in generic stages: {}", targetName);
    ...
    REFERENCE_CAMERA_LOG_INFO("Established frontend subscription: {} (stream: {}) -> {}", frontendName, streamId,
                              targetName);
    return *this;
}
...
void PipelineBuilder::validateStagesUsage()
{
    for (const auto &pair : m_allStages)
    {
        ...
        if (!used)
        {
            REFERENCE_CAMERA_LOG_WARN("Stage {} was added but is not used in any connection or subscription.",
                                      stageName);
            throw std::invalid_argument("Stage " + stageName +
                                        " was added but not used in any connection or subscription.");
        }
...
void PipelineBuilder::validate_and_add_stage(const std::string &name, StagePtr stage, StageType type)
{
    ...
    if (std::dynamic_pointer_cast<FrontendStage>(stage))
    {
        m_frontendStages[name] = std::dynamic_pointer_cast<FrontendStage>(stage);
    }
    else
    {
        m_genericStages[name] = std::dynamic_pointer_cast<ConnectedStage>(stage);
    }

    m_allStages[name] = std::dynamic_pointer_cast<ConnectedStage>(stage);
}
...
std::shared_ptr<Pipeline> PipelineBuilder::build()
{
    ...
    {
        REFERENCE_CAMERA_LOG_ERROR("Invalid state in build. Expected CONNECTING or BUILDING, but current state is {}",
                                   static_cast<int>(m_state));
        throw std::invalid_argument("PipelineBuilder is not in CONNECTING state");
    }
