...
DemuxerStage::DemuxerStage(std::string name, std::string main_outlet_name, std::string sub_outlet_name,
                           size_t queue_size, bool leaky, bool print_fps, bool copy_roi_metadata)
    : ConnectedStage(name, queue_size, leaky, print_fps, true), m_main_outlet_name(main_outlet_name),
      m_sub_outlet_name(sub_outlet_name), m_copy_roi_metadata(copy_roi_metadata)
{
}
...
AppStatus DemuxerStage::process(BufferPtr buffer)
{
    ...
    for (auto &metadata : metadata_list)
    {
        ...
        if (buffer_metadata != nullptr)
        {
            ...
            if (sub_buffer != nullptr)
            {
                ...
                sub_buffer->add_time_stamp(m_stage_name + "_sub");
                send_to_specific_subsciber(m_sub_outlet_name, sub_buffer);
    ...
    send_to_specific_subsciber(m_main_outlet_name, buffer);

    return AppStatus::SUCCESS;
}

AppStatus DemuxerStage::init()
{
    return AppStatus::SUCCESS;
}

AppStatus DemuxerStage::deinit()
{
    return AppStatus::SUCCESS;
}
...
DemuxerStageBuild::Builder &DemuxerStageBuild::Builder::set_printfps_opt(bool print)
{
    m_print_fps = print;
    return *this;
}
...
std::shared_ptr<DemuxerStage> DemuxerStageBuild::Builder::buildptr() const
{
    ...
    return std::make_shared<DemuxerStage>(m_stage_name.value(), m_main_outlet_name.value(), m_sub_outlet_name.value(),
                                          m_queue_size, m_leaky, m_print_fps, m_copy_roi_metadata);
}
