...
public:
    ...
    virtual const char* type_name() const { return "UserDataBase"; }
...
class CameraAppConstructor : public std::enable_shared_from_this<CameraAppConstructor>
{
  public:
    ...
    template <typename T>
    std::shared_ptr<T> get_extension() 
    {
    ...
    static tl::expected<std::shared_ptr<T>, CamAppReturnCode> create(
        std::optional<AppConfigOverride> override_config = std::nullopt)
    {
        if (!std::is_base_of<CameraAppConstructor, T>::value)
        {
            REFERENCE_CAMERA_LOG_ERROR("{} must inherit from AppConstructor", demangle(typeid(T).name()));
  ...
  private:
    ...
    tl::expected<bool, CamAppReturnCode> initialize();
};
