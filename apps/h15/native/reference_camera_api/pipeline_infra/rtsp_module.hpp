...
{
    if (!m_server.get()) return AppStatus::CONFIGURATION_ERROR;
    ...
    GstRTSPMountPoints* mounts = gst_rtsp_server_get_mount_points(m_server.get());
    gst_rtsp_mount_points_add_factory(mounts, m_mount_point.c_str(), m_factory);
    g_object_unref(mounts);

    g_signal_connect(m_server.get(), "client-connected", G_CALLBACK(client_connected), this);
