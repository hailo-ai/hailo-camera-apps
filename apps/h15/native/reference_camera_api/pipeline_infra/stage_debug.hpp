...
class StageDebugCounters
{
  public:
    ...
    StageDebugCounters(const std::string &stage_name)
        : stage_name(stage_name), num_input_frames(0), num_dropped_frames(0), num_output_frames(0),
          num_available_buffers(0), num_failed_acquire_buffer(0)
    {
    }

    // Setters and Getters for the base counters
    void increment_input_frames()
    {
        num_input_frames++;
    }
    ...
    void increment_output_frames()
    {
        num_output_frames++;
    }
    void increment_available_buffers()
    {
        num_available_buffers++;
    }
    void increment_failed_acquire_buffer()
    {
        num_failed_acquire_buffer++;
    }

    void increment_by_val_input_frames(int count)
    {
        num_input_frames += count;
    }
    void increment_by_val_dropped_frames(int count)
    {
        num_dropped_frames += count;
    }
    void increment_by_val_output_frames(int count)
    {
        num_output_frames += count;
    }
    void increment_by_val_available_buffers(int count)
    {
        num_available_buffers += count;
    }
    void increment_by_val_failed_acquire_buffer(int count)
    {
        num_failed_acquire_buffer += count;
    }

    void set_input_frames(int count)
    {
        num_input_frames = count;
    }
    void set_dropped_frames(int count)
    {
        num_dropped_frames = count;
    }
    void set_output_frames(int count)
    {
        num_output_frames = count;
    }
    void set_available_buffers(int count)
    {
        num_available_buffers = count;
    }
    void set_failed_acquire_buffer(int count)
    {
        num_failed_acquire_buffer = count;
    }
