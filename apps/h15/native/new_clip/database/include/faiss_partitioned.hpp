...
class PartitionedFaissDB
{
                                                              ...
                                                              bool auto_shard_seal, size_t hot_shard_max_size)
    {
        ...
        for (const auto &file_path : initial_cold_files)
        {
            ...
            if (!shard_result)
            {
                ...
                std::cout << "WARNING: Error loading initial cold shard: " << file_path << " (" << shard_result.error().message
                          << ")" << ", will ignore this file and continue" << std::endl;
                continue;
            }
        ...
        std::cout << "Partitioned DB created. Loaded " << db->m_cold_shards.size() << " cold shards." << std::endl;
        return db;
    }
  ...
  private:
    ...
    Result<void> insert(const std::vector<float> &vector, faiss::idx_t id)
    {
        ...
        {
            std::cout << "Hot shard threshold reached. Sealing..." << std::endl;
            seal_hot_shard();
        }
    ...
    void rebuild_search_index()
    {
        ...
        std::cout << "Search index rebuilt. Total vectors searchable: " << m_search_index->ntotal << std::endl;
    }
