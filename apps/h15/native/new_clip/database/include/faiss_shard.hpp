  ...
  public:
    ...
    static Result<std::shared_ptr<FaissShard>> create(int dimension, const std::string &index_file_path = "")
    {
        ...
        if (!index_file_path.empty())
        {
            ...
            if (load_result)
            {
                std::cout << "Loaded existing shard from: " << index_file_path << std::endl;
                return shard;
            }
            else
            {
                return tl::unexpected(Error(ErrorType::FILE_LOAD_ERROR, index_file_path + ": " + load_result.error().message));
            }
        }
        else
        {
