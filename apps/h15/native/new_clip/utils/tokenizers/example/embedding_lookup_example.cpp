...
#define CLIP_TEXT_ENCODER_HEF   "/data/clip_vit_b_32_text_encoder_yuval_2025_07_08.hef" // "/data/clip_vit_b_32_text_encoder_mz.hef"  // "/data/clip_resnet_50x4_text_encoder_h15h.hef"
#define CLIP_TEXT_ENCODER_DIM   (512) //(512)   // (640)

#define EMBEDDING_LOOKUP_PATH   "/data/clip_vit_b32_embedding.bin"  // "/data/clip_vit_b32_embedding.bin"  // "/data/clip_resnet50x4_embedding.bin"
...
struct EmbeddingTable {
    ...
    const float* operator[](int token_id) const {
        if (token_id < 0 || static_cast<uint32_t>(token_id) >= rows) return nullptr;
...
void save_as_npy(const std::vector<float>& data, int batch_size, int seq_len, int dim,
...
std::vector<float> build_hailo_input_tensor(const std::vector<int>& tokens,
                                      ...
                                      int max_len = 77)
{
    ...
    // Add sentence tokens
    for (int t : tokens) {
        if ((int)token_ids.size() >= max_len - 1) break; // leave space for end token
...
std::vector<float> apply_text_projection(
...
{
    std::cout << "Running inference with input tensor of size: " 
              << input_tensor.size() << " floats\n";
    ...
    for (size_t i = 0; i < input_tensor.size(); i++) {
        int32_t q = static_cast<int32_t>(std::round(input_tensor[i] / input_quant_info[0].qp_scale + input_quant_info[0].qp_zp));
        if (q < 0) q = 0;
        if (q > 65535) q = 65535; // Clamp to uint16 range
    ...
    for (const auto &input_name : infer_model->get_input_names()) {
        ...
        std::cout << "Allocated input buffer for " << input_name 
                  << " of size " << input_frame_size << " bytes\n";
        ...
        auto input_mapping = DmaMappedBuffer::create(*vdevice, input_buffer_guard.get(), input_frame_size, HAILO_DMA_BUFFER_DIRECTION_H2D).expect("Failed to map input buffer to VDevice");
    ...
    for (const auto &output_name : infer_model->get_output_names()) {
        ...
        std::cout << "Allocated output buffer for " << output_name 
                  << " of size " << output_frame_size << " bytes\n";

        output_buffer_guards.push_back(output_buffer_guard);
        auto output_mapping = DmaMappedBuffer::create(*vdevice, output_buffer_guard.get(), output_frame_size, HAILO_DMA_BUFFER_DIRECTION_D2H).expect("Failed to map output buffer to VDevice");
        output_buffer_map_guards.push_back(std::move(output_mapping));

        auto status = bindings.output(output_name)->set_buffer(MemoryView(output_buffer_guard.get(), output_frame_size));
        if (HAILO_SUCCESS != status) {
            throw hailort_error(status, "Failed to set infer output buffer");
        }
    }

    auto job = configured_infer_model.run_async(bindings, [&output_buffer_guards, &output_quant_info] (const AsyncInferCompletionInfo &completion_info) {
        // check infer status
        ...
        std::memcpy(text_embedding.data(), &float_ptr[final_text_embedding_pos], CLIP_TEXT_ENCODER_DIM * sizeof(float));
#else
        ...
        for (size_t i = 0; i < CLIP_TEXT_ENCODER_DIM; i++) {
            text_embedding[i] = (static_cast<float>(uint8_ptr[final_text_embedding_pos + i]) - output_quant_info[0].qp_zp) * output_quant_info[0].qp_scale;
        }
        ...
        for (size_t i = 0; i < CLIP_TEXT_ENCODER_DIM * 77; i++) {
            output_npy[i] = (static_cast<float>(uint8_ptr[i]) - output_quant_info[0].qp_zp) * output_quant_info[0].qp_scale;
        }
#endif
        ...
        std::vector<float> projected_embedding = apply_text_projection(text_embedding, text_projection_weights, text_projection_bias);  
...
int main() {
    ...
    std::cout << "Loaded embedding table: " 
              << table.rows << " tokens x " << table.cols << " dims\n";
    ...
    std::cout << "Embedding lookup size: " 
              << sentence_embedding.size() << " floats\n";
