...
void ClipAppConfigParser::parse_network_configs(const ryml::ConstNodeRef &root)
{
    ...
    // Clear default list if we're loading from config
    m_config.text_encoder_support_list.clear();
    ...
    for (const auto &text_encoder_node : text_encoders_node)
    {
        ...
        m_config.text_encoder_support_list.push_back(text_encoder);
    }
}
...
void ClipAppConfigParser::parse_pipeline_config(const ryml::ConstNodeRef &root)
{
    ...
    {
        std::cout << "No 'clip_quality_check_stage' section found in 'pipeline_config', using defaults"
                  << std::endl;
    }
    ...
    {
        std::cout << "No 'tracker_traffic_ctrl_stage' section found in 'pipeline_config', using defaults"
                  << std::endl;
    }
