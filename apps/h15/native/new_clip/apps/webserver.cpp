...
constexpr const char* WEBFRONTEND_PATH = "/home/root/apps/new_clip/resources/webfrontend/index.html";

// WebRtcStreamers constructor implementation
IntegratedWebServer::WebRtcStreamers::WebRtcStreamers(StreamType t, const std::string &id, std::shared_ptr<WebRTCStreamerExt> ext)
...
IntegratedWebServer::IntegratedWebServer(ClipAppConfig config) 
    : server(std::make_unique<httplib::Server>()),
      web_server_config(std::make_unique<ClipAppConfig>(config))
{
}
...
tl::expected<std::string, IntegratedWebServer::ImageError> IntegratedWebServer::loadJpegFile(const std::string &filepath)
{
...
tl::expected<void, IntegratedWebServer::ImageError> IntegratedWebServer::addImage(const std::string &jpeg_path, const std::string &description,
...
std::shared_ptr<IntegratedWebServer::WebRtcStreamers> IntegratedWebServer::findWebRtcStreamer(WebRtcStreamers::StreamType type)
{
...
std::shared_ptr<IntegratedWebServer::WebRtcStreamers> IntegratedWebServer::findWebRtcStreamerBySessionId(const std::string &session_id)
{
    auto it =
...
tl::expected<std::shared_ptr<IntegratedWebServer>, std::string> IntegratedWebServer::create(const ClipAppConfig &config)
{
    ...
    instance->m_webrtc_streamers.emplace_back(
        WebRtcStreamers::StreamType::QUERY_PLAYBACK,
        instance->clip_query_player_streaming_service->get_webrtc_streamer()->get_session_id(),
...
void IntegratedWebServer::validateModelFiles()
{
    ...
    for (const auto &network : web_server_config->text_encoder_support_list)
    {
        ...
        {
            std::cout << "✗ Missing ONNX model (Can still work if text encode on browser is disabled): " << network.network_name << " at " << model_path << std::endl;
        }
    }
}
...
void IntegratedWebServer::setupRoutes()
{
    ...
    server->Post("/api/config/video_playback_total_length",
        [this](const httplib::Request &req, httplib::Response &res) {
    ...
    server->Get("/api/models/([^/]+)", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        std::cout << "Streaming ONNX model: " << model_path << " (" << file_size << " bytes) in chunks"
                  << std::endl;
    });
    ...
    server->Post("/api/webrtc/session-video-thumbnail", [this](const httplib::Request &req, httplib::Response &res) {
    ...
    server->Post("/api/webrtc/offer", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        try
        {
            ...
            std::string offer = webrtc_streamer_ext->streamer_ext->create_offer();
    ...
    server->Post("/api/webrtc/answer", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        try
        {
            ...
            webrtc_streamer_ext->streamer_ext->handle_answer(answerJson["sdp"]);
    ...
    server->Post("/api/webrtc/ice-candidate", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        try
        {
            ...
            webrtc_streamer_ext->streamer_ext->handle_ice_candidate(
    ...
    server->Get("/api/webrtc/status", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        statusJson["connected"] =
            (webrtc_streamer_ext) ? !webrtc_streamer_ext->streamer_ext->is_connection_closed() : false;
    ...
    server->Post("/api/webrtc/video-thumbnail-stop", [this](const httplib::Request &req, httplib::Response &res) {
        ...
        webrtc_streamer_ext->streamer_ext->close_connection();
