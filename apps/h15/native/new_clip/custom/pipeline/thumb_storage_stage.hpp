...
#include "stage.hpp"
#include "buffer.hpp"
#include "database/include/thumbnail_table.hpp"
...
class ThumStorageStage : public ConnectedStage
{
  ...
  public:
    inline ThumStorageStage(std::string name, DBSource db_source, std::string db_source_data, std::string thumb_dir,
        ...
        : ConnectedStage(name, queue_size, leaky, print_fps), m_db_source(db_source), m_db_source_data(db_source_data),
          m_thumb_dir(thumb_dir), m_thumb_filename_prefix(thumb_filename_prefix), m_should_terminate(false)
    {
    }
    ...
    inline AppStatus init() override
    {
        ...
        case DB_SOURCE_FROM_FACTORY: {
            ...
            if (!thumbnail_table_result)
            {
                ...
                REFERENCE_CAMERA_LOG_ERROR("thumbnail Storage {} database not found in factory", m_stage_name);
                return AppStatus::UNINITIALIZED;
            }
        ...
        default:
            ...
            REFERENCE_CAMERA_LOG_ERROR("Thumbnail Storage {} unsupported faiss database source", m_stage_name);
    ...
    inline AppStatus process(BufferPtr data)
    {
        ...
        if (m_thumb_table == nullptr)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Thumb Storage {} database failed to initialized", m_stage_name);
            return AppStatus::UNINITIALIZED;
        }
        ...
        if (metadata.size() <= 0)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Thumb Storage {} got buffer of unknown size, add SizeMeta");
            return AppStatus::PIPELINE_ERROR;
        }

        // Generate filename based on epoch milliseconds and we make sure it's unique as sometime file saving is too fast
        // and we could endp up with same filename.
        ...
        if (!file)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Thumb Storage {} failed to open file {}/{}", m_stage_name, thumb_path,
                                    filename);
            return AppStatus::PIPELINE_ERROR;
        }
  ...
  private:
            ...
            if (!m_thumb_cache_dir.empty())
            {
                ...
                for (const auto &item : thumbnails_to_process)
                {
                    ...
                    if (ret != 0)
                    {
                        ...
                        REFERENCE_CAMERA_LOG_ERROR("Thumb Storage {} failed to move file from {} to {}, error code: {}", m_stage_name,
                                                cache_path, final_path, ret);
                    }
                }
                ...
                if (duration.count() > 30)
                {
                    std::cout << "Time taken THUMB image move from cache to storage: " << duration.count() << " ms" << std::endl;
                }        
    ...
    bool write_encoded_data(HailoMediaLibraryBufferPtr buffer, uint32_t size, std::ofstream &output_file)
    {
        ...
        if (!data)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Thumb Storage {} write encoded data", m_stage_name);
            return false;
        }
