...
#include "stage.hpp"
#include "buffer.hpp"
#include "database/include/faiss_table.hpp"
#include "database/faiss_factory.hpp"
...
class FaissStorageStage : public ConnectedStage
{
  public:
  ...
  public:
    inline FaissStorageStage(std::string name, std::string db_path, std::string faiss_dir,
        ...
        : ConnectedStage(name, queue_size, leaky, print_fps), m_db_path(db_path), m_faiss_dir(faiss_dir),
          m_faiss_filename_prefix(faiss_filename_prefix), m_should_terminate(false)
    {
    ...
    inline FaissStorageStage(std::string name, std::string db_factory_name,
        ...
        : ConnectedStage(name, queue_size, leaky, print_fps), m_db_factory_name(db_factory_name),
    ...
    inline AppStatus init() override
    {
        ...
        case DB_SOURCE_FROM_FACTORY: {
            ...
            if (!faiss_table_result)
            {
                ...
                REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} database not found in factory", m_stage_name);
        ...
        default:
            ...
            REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} unsupported faiss database source", m_stage_name);
            ...
            for (auto &queue : m_queues)
            {
                if (queue->size())
                {
                    ...
                    else
                    {
                        ...
                        REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} failed to process data", m_stage_name);
                    }
    ...
    inline AppStatus process(BufferPtr data)
    {
        if (m_faiss_table == nullptr)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} database failed to initialized", m_stage_name);
            return AppStatus::UNINITIALIZED;
        }
        ...
        if (track_id == -1)
        {
            ...
            REFERENCE_CAMERA_LOG_WARN("Faiss Storage {} tracking ID not found, skipping data", m_stage_name);
            return AppStatus::SUCCESS;
        }
        ...
        default:
            ...
            REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} unsupported faiss index source", m_stage_name);
            return AppStatus::CONFIGURATION_ERROR;
        }
        ...
        if (faiss_index == nullptr)
        {
            ...
            REFERENCE_CAMERA_LOG_ERROR("Faiss Storage {} faiss index object not found", m_stage_name);
        ...
        if (roi->get_objects_typed(HAILO_MATRIX).size() > 1)
        {
            std::cout << "WARNING: Faiss Storage " << m_stage_name << " multiple matrix found, should only be one"
                      << std::endl;
            REFERENCE_CAMERA_LOG_WARN("Faiss Storage {} multiple matrix found, should only be one", m_stage_name);
        }
    ...
    std::pair<std::string, FaissDatabaseQuickAccess::DatabasePtr> get_faiss_index_from_user_meta(HailoROIPtr roi)
    {
        ...
        if (roi->get_objects_typed(HAILO_USER_META).size() > 1)
        {
            std::cout << "WARNING: Faiss Storage " << m_stage_name << " multiple user meta found, should only be one"
                      << std::endl;
            REFERENCE_CAMERA_LOG_WARN("Faiss Storage {} multiple user meta found, , should only be one", m_stage_name);
        }
        ...
        if (network_embedding_name.empty() || network_embedding_size == 0)
        {
            std::cout << "Faiss Storage " << m_stage_name << " network embedding name or size not found, skipping data"
                      << std::endl;
            REFERENCE_CAMERA_LOG_WARN("Faiss Storage {} network embedding name or size not found, skipping data",
                                      m_stage_name);
