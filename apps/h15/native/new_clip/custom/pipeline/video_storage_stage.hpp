...
#include "stage.hpp"
#include "buffer.hpp"
#include "database/include/video_table.hpp"
...
class VideoStorageStage : public ConnectedStage
{
  public:
    enum DBSource
    {
  ...
  public:
    inline VideoStorageStage(std::string name, DBSource db_source, std::string db_source_data, std::string video_dir,
        ...
        : ConnectedStage(name, queue_size, leaky, print_fps), m_db_source(db_source), m_db_source_data(db_source_data),
          m_video_dir(video_dir), m_video_filename_prefix(video_filename_prefix),
