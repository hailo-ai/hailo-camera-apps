...
class ClipQueryServiceExt : public CameraAppExtension
{
  public:
    ...
    tl::expected<void, ClipQueryErrCode> configure(std::string database_file_path, const DatabaseConfig &db_config, std::shared_ptr<TextEncoder> text_encoder)
    {
        if (m_is_configured)
        {
            return {};
    ...
    tl::expected<std::vector<ThumbQueryResult>, ClipQueryErrCode> query_thumbnails_on_device(QueryEmbeddingInfo &query_data)
    {
        if (!m_is_configured)
        {
            return tl::unexpected(ClipQueryErrCode::MISSING_CONFIGURATION);
    ...
    tl::expected<std::vector<ThumbQueryResult>, ClipQueryErrCode> query_thumbnails_with_embedding(QueryEmbeddingInfo &query_data)
    {
        if (!m_is_configured)
        {
            return tl::unexpected(ClipQueryErrCode::MISSING_CONFIGURATION);
  ...
  private:
     ...
     * @param query_data The query data containing information such as network ID and prompt that is required in aggregated data
     * @return A new vector containing the aggregated data
     */    
    std::vector<PositiveQueryFullRecord> process_for_positive_query_results(std::vector<PartitionedFaissDB::SearchResult> &faiss_index_search_result, 
                                                                            QueryEmbeddingInfo &query_data)
    {
        ...
        for (const auto &thumbnail_result : thumbnail_table_results)
        {
            ...
            auto faiss_table_it = std::find_if(faiss_table_results.begin(), faiss_table_results.end(),
            ...
            if (faiss_table_it != faiss_table_results.end())
            {
                ...
                if (faiss_search_it != faiss_index_search_result.end())
                {
                    ...
                    positive_query_results.emplace_back(faiss_id, track_id,
                                                        timestamp, cosine_sim,
                                                        query_data.m_positive_embedding.prompt,
                                                        query_data.m_network_id, thumbnail_path);
                }
            }
        }
