...
public:
    ...
    struct TextEncoderConfig
    {
        ...
        TextEncoderConfig(const std::string &hailort_device_id,
                          ...
                          int                emb_dim)
            : hailort_device_id(hailort_device_id),
              network_id(network_id),
              token_file_path(token_path),
              embedding_look_up_file_path(embedding_path),
              projection_weight_file_path(weight_path),
              projection_bias_file_path(bias_path),
        ...
        for (const auto &cfg : m_config)
        {
            ...
            hailort_service->register_output_callback(
                [this, network_id = cfg.network_id](std::vector<float> output_embedding) {
                    ...
                    //AARON DEBUG ONLY
                    //save_as_npy(output_embedding, 1, 77, 512, network_id + "_output_embedding_before_projection.npy");
            ...
            if (!embedding_lookup.load(cfg.embedding_look_up_file_path)) {
                std::cerr << "Failed to load embedding look up from file: " << cfg.embedding_look_up_file_path << std::endl;
                return tl::unexpected(ErrorCode::INVALID_PARAMETER);
            }
            ...
            if (!projection_weights.load(cfg.projection_weight_file_path)) {
                std::cerr << "Failed to load projection weights from file: " << cfg.projection_weight_file_path << std::endl;
                return tl::unexpected(ErrorCode::INVALID_PARAMETER);
            }
                                                      ...
                                                      const std::vector<std::string> &negative_prompts = {}) override
    {
        ...
        for (const auto &pos_prompt : positive_prompts)
        {
            if (index < m_output_embeddings.size()) {
                ...
                std::vector<float> final_text_embedding(m_output_embeddings[index].begin() + eot_start_pos, m_output_embeddings[index].begin() + eot_end_pos);
...
private:
    ...
    std::vector<float> build_sentence_embedding(const std::vector<int>& tokens,
                                                const ClipBinMatrix& embedding_lookup_table,
                                                int max_len = 77)
    {
    ...
    void worker_thread_function() {
        ...
        while (m_thread_running) {
            ...
            m_thread_cv.wait_for(lock, std::chrono::seconds(1), [this]() { 
