...
void MKVStreamer::process_gst_sample(GstSample *original_sample, FileContext *context)
{
    ...
    // 1. Create a deep, independent copy of the buffer. This new buffer is
    //    guaranteed to be writable and has no ties to the upstream memory pool.
    GstBuffer *writable_buffer = gst_buffer_copy_deep(original_buffer);
    if (!writable_buffer)
    {
    ...
    // Also populate the other fields for logging/debugging purposes.
    // Re-map the buffer (now owned by new_sample) to read the final data.
    GstBuffer *final_buffer = gst_sample_get_buffer(new_sample);
    gst_buffer_map(final_buffer, &map_info, GST_MAP_READ);

    packet_data.data = map_info.data;
    packet_data.size = map_info.size;

    if (map_info.size >= 12)
    {
        uint8_t *rtp_header = map_info.data;
        packet_data.marker_bit = (rtp_header[1] & 0x80) != 0;
        packet_data.payload_type = rtp_header[1] & 0x7F;
        packet_data.sequence_number = g_ntohs(*((uint16_t *)(rtp_header + 2)));
        packet_data.timestamp = g_ntohl(*((uint32_t *)(rtp_header + 4)));
        packet_data.ssrc = g_ntohl(*((uint32_t *)(rtp_header + 8)));
        packet_data.is_keyframe = packet_data.marker_bit;
    }

    packet_data.original_timestamp_ms = context->video_file.start_timestamp_ms;
    packet_data.codec_type = context->detected_codec;

    gst_buffer_unmap(final_buffer, &map_info);
    ...
    // for calling gst_sample_unref() on it when it's done.
    {
        ...
        {
            // If there's no callback, we MUST unref the sample we created to prevent a memory leak.
...
std::unique_ptr<MKVStreamer::FileContext> MKVStreamer::create_file_context(const VideoFile &video_file)
{
    ...
    context->base_rtp_timestamp = m_global_rtp_timestamp;
