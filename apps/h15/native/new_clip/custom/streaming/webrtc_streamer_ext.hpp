...
class WebRTCStreamerExt : public CameraAppExtension
{
  private:
  ...
  public:
    WebRTCStreamerExt(const std::string &session_id = "")
        : m_ssrc(generate_unique_ssrc()), m_session_id(session_id.empty() ? generate_session_id() : session_id)
    {
    ...
    void send_rtp_packet(GstSample *sample)
    {
        ...
        if (m_is_streaming)
            ...
            gst_sample_unref(sample); // Clean up if not streaming
    }
    ...
    void streaming_loop()
    {
        while (m_is_streaming)
        {
            ...
            if (!gst_buffer_map(buffer, &mapInfo, GST_MAP_READ))
            {
                ...
                REFERENCE_CAMERA_LOG_ERROR("WebRtc Streamer Failed to Map Buffer");
                continue;
            }
