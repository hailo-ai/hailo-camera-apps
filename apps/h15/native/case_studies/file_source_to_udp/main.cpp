...
std::shared_ptr<Pipeline> create_pipeline(const PipelineConfig &config,
                                          std::shared_ptr<MediaLibraryEncoder> media_encoder)
{
    ...
    auto encoder = EncoderStageBuild::create().set_stage_name("encoder").set_printfps_opt(config.print_fps).buildptr();

    if (encoder->configure(media_encoder) != AppStatus::SUCCESS)
    {
    ...
    auto udp_output =
        UdpStageBuild::create().set_stage_name("udp_output").set_printfps_opt(config.print_fps).buildptr();
    udp_output->configure(config.host, config.port, EncodingType::H264);

    PipelineBuilder builder;
    builder.add_stage(file_source, StageType::SOURCE)
        .add_stage(encoder, StageType::SINK)
        .add_stage(udp_output, StageType::SINK)
        .connect("file_source", "encoder")
        .connect("encoder", "udp_output");

    return builder.build();
}

void run_pipeline(const PipelineConfig &config, std::shared_ptr<Pipeline> pipeline)
{
