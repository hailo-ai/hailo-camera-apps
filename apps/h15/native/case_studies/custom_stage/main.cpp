...
class CustomStage : public ConnectedStage
{
  public:
    CustomStage(std::string name, size_t queue_size, bool leaky = false, bool print_fps = false)
...
void create_ai_pipeline(std::shared_ptr<AppResources> app_resources)
{
    ...
    std::shared_ptr<OverlayStage> overlay_stage = std::make_shared<OverlayStage>(
        OVERLAY_STAGE, false, true, 0, 1, 1, false, std::unordered_set<int>{}, nullptr, app_resources->print_fps);
