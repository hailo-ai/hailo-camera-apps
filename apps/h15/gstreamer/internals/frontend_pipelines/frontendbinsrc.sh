...
function print_usage() {
    echo "Frontend streaming pipeline usage:"
    echo ""
    echo "Options:"
...
function create_pipeline() {
    ...
    FOUR_K_TO_ENCODER_BRANCH="queue leaky=downstream max-size-buffers=$max_buffers_size max-size-bytes=0 max-size-time=0 ! \
    ...
    FOUR_K_BRANCH="queue leaky=downstream max-size-buffers=$max_buffers_size max-size-bytes=0 max-size-time=0 ! \
                   $FPS_DISP name=hailo_display_4k "

    # In the FHD branch we do not output the stream into Encoder, due to hardware limitation.
    FHD_BRANCH="queue leaky=downstream max-size-buffers=$max_buffers_size max-size-bytes=0 max-size-time=0 ! \
                $FPS_DISP name=hailo_display_fhd "

    HD_BRANCH="queue leaky=downstream max-size-buffers=$max_buffers_size max-size-bytes=0 max-size-time=0 ! \
    ...
    SD_BRANCH="queue leaky=downstream max-size-buffers=$max_buffers_size max-size-bytes=0 max-size-time=0 ! \
...
PIPELINE="${debug_stats_export} gst-launch-1.0 \
    ...
    frontend. ! $FOUR_K_TO_ENCODER_BRANCH \
    frontend. ! $FOUR_K_BRANCH \
    frontend. ! $FHD_BRANCH \
    frontend. ! $HD_BRANCH \
    frontend. ! $SD_BRANCH \
    ${additonal_parameters} "
