...
function init_variables() {
    ...
    readonly DEFAULT_DIS_DEWARP_CONFIG_FILE_PATH="$RESOURCES_DIR/configs/dis_dewarp_config.json"

    input_source=$DEFAULT_VIDEO_SOURCE

    json_config_path_4k="$MEDIA_LIB_RESOURCES_DIR/configs/osd_4k.json"
    
    dis_dewarp_config_file_path="$DEFAULT_DIS_DEWARP_CONFIG_FILE_PATH"
...
function print_usage() {
    echo "DIS and Dewarp streaming pipeline usage:"
    echo ""
    echo "Options:"
    ...
    echo "  --vision-config-file-path  Set the vision config file path (default $DEFAULT_DIS_DEWARP_CONFIG_FILE_PATH)"
    exit 0
}
...
PIPELINE="${debug_stats_export} gst-launch-1.0 \
    ...
    hailodewarp config-file-path=$dis_dewarp_config_file_path name=dis_dewarp ! \
