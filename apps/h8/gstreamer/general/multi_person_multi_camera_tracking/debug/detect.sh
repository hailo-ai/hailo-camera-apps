...
function main() {
    ...
    DETECTION_PIPELINE="queue name=hailo_pre_cropper1_q leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
        ...
        queue name=detector_after_net leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
        hailopython module=$TAPPAS_WORKSPACE/apps/h8/gstreamer/general/multi_person_multi_camera_tracking/debug/record_net_output.py qos=false ! \
    ...
    pipeline="gst-launch-1.0 \
        ...
        hailopython module=$TAPPAS_WORKSPACE/apps/h8/gstreamer/general/multi_person_multi_camera_tracking/debug/json_saver.py qos=false ! \
