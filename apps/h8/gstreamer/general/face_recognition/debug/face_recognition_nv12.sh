...
function init_variables() {
    ...
    readonly DEFAULT_HEF_PATH="$RESOURCES_DIR/scrfd_10g_nv12.hef"
    readonly POSTPROCESS_SO="$POSTPROCESS_DIR/libface_detection_post.so"
    readonly FACE_JSON_CONFIG_PATH="$RESOURCES_DIR/configs/scrfd_nv12.json"
...
function main() {
    ...
    RECOGNITION_PIPELINE="hailocropper so-path=$CROPPER_SO function-name=face_recognition internal-offset=true name=cropper2 \
        ...
        videoscale qos=false ! video/x-raw, width=112, height=112, pixel-aspect-ratio=1/1 ! \
        ...
        hailofilter use-gst-buffer=true so-path=$RECOGNITION_POST_SO name=face_recognition_hailofilter function-name=$RECOGNITION_FUNCTION_NAME qos=false ! \
    ...
    pipeline="gst-launch-1.0 \
        ...
        videorate ! video/x-raw,framerate=90/1 ! \
        videoconvert n-threads=2 qos=false ! video/x-raw, format=NV12, pixel-aspect-ratio=1/1 ! \
