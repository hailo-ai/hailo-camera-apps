...
function create_lp_detection_pipeline() {
    lp_detection="hailocropper so-path=$crop_so function-name=$license_plate_detection_crop_func internal-offset=$internal_offset drop-uncropped-buffers=false name=cropper1 \
                ...
                cropper1. ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                    hailonet hef-path=$lp_hef device-count=1 vdevice-key=1 ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                    hailofilter function-name=$lp_post_name so-path=$vehicle_detection_postprocess_so config-path=$license_plate_json_config_path qos=false ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                agg1. \
...
function create_ocr_pipeline() {
    ocr_pipeline="hailocropper so-path=$crop_so function-name=$ocr_crop_func internal-offset=$internal_offset drop-uncropped-buffers=false name=cropper2 \
                cropper2. ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                    fakesink \
                cropper2. ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! \
                    hailonet hef-path=$ocr_hef device-count=1 vdevice-key=1 is-active=false ! \
                    queue leaky=no max-size-buffers=5 max-size-bytes=0 max-size-time=0 ! "
}
...
function main() {
    ...
    pipeline="gst-launch-1.0 \
        ...
        jpegenc ! filesink location=${TAPPAS_WORKSPACE}/lpr_lp_detection_crop.jpg"
