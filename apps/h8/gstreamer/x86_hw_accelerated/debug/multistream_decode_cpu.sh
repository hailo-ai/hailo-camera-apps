...
function create_sources() {
    ...
    for ((n = $start_index; n < $num_of_src; n++)); do
        sources+="filesrc location=$RESOURCES_DIR/video$n.mp4 name=source_$n ! \
                ...
                decodebin ! \
                queue name=pre_convt_q_$n leaky=no max-size-buffers=30 max-size-bytes=0 max-size-time=0 ! \
		videoconvert ! video/x-raw,format=$format,width=1920,height=1080 ! \
...
function main() {
    ...
    echo "Pipeline (CPU): $resolution - $format - $num_of_src - $fps - $sink_element"
