...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_hailotilecropper_debug, "hailotilecropper", 0, "hailotilecropper element");
...
static GType gst_hailotilecropper_tiling_mode_get_type(void)
{
    ...
    {
        cropper_tiling_mode = g_enum_register_static("GstHailoTileCropperTilingMode", hailotilecropper_tiling_modes);
    }
    return cropper_tiling_mode;
}

static void gst_hailotilecropper_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_hailotilecropper_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
...
void tiling_resize(GstHailoBaseCropper *basecropper, std::vector<cv::Mat> &cropped_image_vec,
                   std::vector<cv::Mat> &resized_image_vec, HailoROIPtr roi, GstVideoFormat image_format);

static void gst_hailotilecropper_class_init(GstHailoTileCropperClass *klass)
{
    ...
    gst_element_class_set_details_simple(gstelement_class, "hailotilecropper - Tiling", "Hailo/Tools",
                                         "Crops and scales an image into tiles", "hailo.ai <contact@hailo.ai>");

    g_object_class_install_property(
        gobject_class, PROP_TILES_X_AXIS,
        g_param_spec_uint("tiles-along-x-axis", "Tiles along x axis", "Number of tiles along x axis (columns)", 1, 20,
                          2, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));

    g_object_class_install_property(
        gobject_class, PROP_TILES_Y_AXIS,
        g_param_spec_uint("tiles-along-y-axis", "Tiles along y axis", "Number of tiles along x axis (rows)", 1, 20, 2,
                          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));

    g_object_class_install_property(
        gobject_class, PROP_OVERLAP_X_AXIS,
        g_param_spec_float("overlap-x-axis", "Overlap x axis",
                           "Overlap in percentage between tiles along x axis (columns)", 0, 1, 0,
                           (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));

    g_object_class_install_property(
        gobject_class, PROP_OVERLAP_Y_AXIS,
        g_param_spec_float("overlap-y-axis", "Overlap y axis",
                           "Overlap in percentage between tiles along y axis (rows)", 0, 1, 0,
                           (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
    ...
    g_object_class_install_property(
        gobject_class, PROP_MULTI_SCALE_LEVEL,
        g_param_spec_uint("scale-level", "Scale level",
                          "Scales (layers of tiles) in addition to the main layer 1: [(1 X 1)] 2: [(1 X 1), (2 X 2)] "
                          "3: [(1 X 1), (2 X 2), (3 X 3)]]",
                          1, 3, 2,
                          (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
}

static void gst_hailotilecropper_init(GstHailoTileCropper *hailotilecropper)
{
 ...
 */
static HailoTileROI create_tile_roi(const uint &index, const float &col_overlap, const float &row_overlap,
                                    const float &xmin, const float &ymin, const float &xmax, const float &ymax,
                                    const uint &layer, hailo_tiling_mode_t tiling_mode)
{
...
static void prepare_tiles(HailoROIPtr hailo_roi, std::vector<HailoROIPtr> &crop_rois, float tiles_along_x_axis,
                          float tiles_along_y_axis, float overlap_x_axis, float overlap_y_axis, uint layer,
                          hailo_tiling_mode_t tiling_mode)
{
    ...
    while (float(row_offset + row_step) <= 1)
    {
        ...
        {
            // Create new tile ROI
            HailoTileROIPtr tile_roi = std::make_shared<HailoTileROI>(
                create_tile_roi(index, col_overlap, row_overlap, col_offset, row_offset, (col_offset + col_step),
                                (row_offset + row_step), layer, tiling_mode));
...
static std::vector<HailoROIPtr> gst_hailotilecropper_prepare_crops(GstHailoBaseCropper *hailocropper, GstBuffer *buf)
{
    ...
    if (hailotilecropper->tiling_mode == MULTI_SCALE)
        for (uint i = 0; i < num_of_scales; i++)
            prepare_tiles(hailo_roi, crop_rois, scales_template[i][0], scales_template[i][1],
                          hailotilecropper->overlap_x_axis, hailotilecropper->overlap_y_axis, (i + 1),
                          (hailo_tiling_mode_t)hailotilecropper->tiling_mode);
