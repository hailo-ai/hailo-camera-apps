...
{
    if (class_name == "HailoClassification")
        return HAILO_CLASSIFICATION;
    if (class_name == "HailoDetection")
        return HAILO_DETECTION;
    if (class_name == "HailoLandmarks")
        return HAILO_LANDMARKS;
    if (class_name == "HailoTileROI")
        return HAILO_TILE;
    if (class_name == "HailoUniqueID")
...
{
    return HailoBBox(object_json["xmin"].GetFloat(), object_json["ymin"].GetFloat(), object_json["width"].GetFloat(),
                     object_json["height"].GetFloat());
}

inline HailoPoint decode_point(rapidjson::Value &object_json)
{
    return HailoPoint(object_json["x"].GetFloat(), object_json["y"].GetFloat(), object_json["confidence"].GetFloat());
}
...
inline void decode_detection(rapidjson::Value &object_json, HailoROIPtr roi)
{
    ...
    HailoDetectionPtr detection = std::make_shared<HailoDetection>(
        bbox, object_json["class_id"].GetInt(), object_json["label"].GetString(), object_json["confidence"].GetFloat());
...
inline void decode_classification(rapidjson::Value &object_json, HailoROIPtr roi)
{
    ...
    roi->add_object(std::make_shared<HailoClassification>(
        object_json["classification_type"].GetString(), object_json["class_id"].GetInt(),
        object_json["label"].GetString(), object_json["confidence"].GetFloat()));
}
...
inline void decode_matrix(rapidjson::Value &object_json, HailoROIPtr roi)
{
    ...
    roi->add_object(std::make_shared<HailoMatrix>(matrix_data, object_json["height"].GetInt(),
                                                  object_json["width"].GetInt(), object_json["features"].GetInt()));
}
...
inline void decode_landmarks(rapidjson::Value &object_json, HailoROIPtr roi)
{
    ...
    roi->add_object(std::make_shared<HailoLandmarks>(object_json["landmarks_type"].GetString(), decoded_points,
                                                     object_json["threshold"].GetFloat(), decoded_pairs));
}
...
inline void decode_tile(rapidjson::Value &object_json, HailoROIPtr roi)
{
    ...
    HailoTileROIPtr tile =
        std::make_shared<HailoTileROI>(bbox, object_json["index"].GetInt(), object_json["overlap_x_axis"].GetFloat(),
                                       object_json["overlap_y_axis"].GetFloat(), object_json["layer"].GetInt(),
...
inline void decode_hailo_face_recognition_result(rapidjson::Value object_json, HailoROIPtr roi,
