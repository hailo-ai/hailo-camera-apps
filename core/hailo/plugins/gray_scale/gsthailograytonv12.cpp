...
static GstFlowReturn gst_hailograytonv12_prepare_output_buffer(GstBaseTransform *trans, GstBuffer *inbuf,
                                                               GstBuffer **outbuf);
GstCaps *gst_hailograytonv12_transform_caps(GstBaseTransform *trans, GstPadDirection direction, GstCaps *caps,
                                            GstCaps *filter);
...
static void gst_hailograytonv12_class_init(GstHailograytonv12Class *klass)
{
    ...
    gst_element_class_set_static_metadata(
        GST_ELEMENT_CLASS(klass), "hailograytonv12 - postprocessing element", "Hailo/Tools",
        "Converts GRAY8 to NV12 by using the metadata that contains a pointer to the original NV12 buffer",
...
static GstFlowReturn gst_hailograytonv12_prepare_output_buffer(GstBaseTransform *trans, GstBuffer *inbuf,
                                                               GstBuffer **outbuf)
{
    // here we want to get the meta data that has the pointer to the original NV12 buffer. we are looking for the
    // GstParentBufferMeta that doesn't have the tensor meta
