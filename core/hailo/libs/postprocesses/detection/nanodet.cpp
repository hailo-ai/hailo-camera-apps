                                                                                  ...
                                                                                  int regression_length)
{
    ...
    {
        total_scores += tensors[i]->width() * tensors[i]->height();
    }
    ...
    for (uint i = 0; i < tensors.size(); i++)
    {
        ...
        auto layer = common::dequantize(common::get_xtensor(tensors[i]), tensors[i]->qp_scale(), tensors[i]->qp_zp());
        ...
        xt::view(scores, xt::range(view_index, view_index + num_proposals), xt::all()) =
            xt::reshape_view(layer_scores, {num_proposals, num_classes});
 ...
 */
std::vector<HailoDetection> decode_boxes(std::vector<xt::xarray<float>> raw_boxes, xt::xarray<float> scores,
                                         std::vector<int> network_dims, std::vector<int> strides, int regression_length)
{
    ...
    for (uint i = 0; i < raw_boxes.size(); i++)
    {
        ...
        auto regression_distance =
            xt::reshape_view(xt::arange(0, regression_length + 1), {1, 1, regression_length + 1});
        ...
        {
            HailoBBox bbox(decoded_boxes(j, 0) / network_dims[0], decoded_boxes(j, 1) / network_dims[1],
 ...
 */
std::vector<HailoDetection> nanodet_postprocess(std::vector<HailoTensorPtr> &tensors, std::vector<int> network_dims,
...
void nanodet_repvgg(HailoROIPtr roi)
{
    ...
    std::vector<HailoDetection> detections =
        nanodet_postprocess(tensors, network_dims, strides, regression_length, NUM_CLASSES);
