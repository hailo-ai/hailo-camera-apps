...
class FaceDetectionParams
{
  public:
    ...
    FaceDetectionParams(xt::xarray<float> anchors, xt::xarray<float> anchors_multiplier,
                        xt::xarray<float> anchor_variance, std::vector<std::vector<int>> anchor_min_size,
                        float score_threshold, float iou_threshold, int num_branches)
    {
