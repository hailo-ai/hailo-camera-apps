...
void add_attribute_prediction_to_roi(HailoROIPtr roi, std::vector<HailoUniqueIDPtr> unique_ids,
                                     std::string jde_tracker_name, std::string label, float confidence, int index)
{
    ...
    if (confidence > RESNET_V1_18_FACE_THRESHOLD)
    {
        if (label != "No_Beard")
        {
            ...
            classification =
                std::make_shared<HailoClassification>(std::string("face_attributes"), index, label, confidence);
        }
    }
    else
    {
        ...
        if (new_label != "")
        {
            // Create the classification result
            classification =
                std::make_shared<HailoClassification>(std::string("face_attributes"), index, new_label, 0.99f);
        }
    }
