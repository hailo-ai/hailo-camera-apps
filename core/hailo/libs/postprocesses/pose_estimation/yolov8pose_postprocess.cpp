...
std::vector<std::pair<int, int>> JOINT_PAIRS = {{0, 1},   {1, 3},   {0, 2},   {2, 4},  {5, 6},  {5, 7},
                                                {7, 9},   {6, 8},   {8, 10},  {5, 11}, {6, 12}, {11, 12},
                                                {11, 13}, {12, 14}, {13, 15}, {14, 16}};

std::pair<std::vector<KeyPt>, std::vector<PairPairs>> process_single_decoding(const Decodings &dec,
                                                                              ...
                                                                              float joint_threshold = 0.5)
{
    ...
    for (uint i = 0; i < score.shape(0); i++)
    {
        ...
        {
            keypoints.push_back(
                KeyPt({coordinates(i, 0) / network_dims[0], coordinates(i, 1) / network_dims[1], score(i, 0)}));
        }
    }
    ...
    for (const auto &pair : JOINT_PAIRS)
    {
        ...
        {
            PairPairs pr = PairPairs({std::make_pair(coordinates(pair.first, 0) / network_dims[0],
                                                     coordinates(pair.first, 1) / network_dims[1]),
                                      std::make_pair(coordinates(pair.second, 0) / network_dims[0],
                                                     coordinates(pair.second, 1) / network_dims[1]),
                                      score(pair.first, 0), score(pair.second, 0)});
            pairs.push_back(pr);
        }
    }
...
std::pair<std::vector<KeyPt>, std::vector<PairPairs>> filter_keypoints(const std::vector<Decodings> &filtered_decodings,
                                                                       const std::vector<int> &network_dims,
                                                                       float joint_threshold = 0.5)
{
...
std::vector<Decodings> nms(std::vector<Decodings> &decodings, const float iou_thr,
                           bool should_nms_cross_classes = false)
{
    ...
    for (uint index = 0; index < decodings.size(); index++)
    {
        if (decodings[index].detection_box.get_confidence() != 0.0f)
        {
            ...
            {
                if ((should_nms_cross_classes || (decodings[index].detection_box.get_class_id() ==
                                                  decodings[jindex].detection_box.get_class_id())) &&
    ...
    for (uint index = 0; index < decodings.size(); index++)
    {
        if (decodings[index].detection_box.get_confidence() != 0.0f)
        {
            decodings_after_nms.push_back(
                Decodings{decodings[index].detection_box, decodings[index].keypoints, decodings[index].joint_pairs});
        }
    }
...
void dequantize_box_values(xt::xarray<float> &dequantized_outputs, int index, xt::xarray<uint8_t> &quantized_outputs,
...
std::vector<Decodings> decode_boxes_and_keypoints(std::vector<HailoTensorPtr> raw_boxes_outputs,
                                                  xt::xarray<float> scores, std::vector<HailoTensorPtr> raw_keypoints,
                                                  std::vector<int> network_dims, std::vector<int> strides,
                                                  int regression_length)
{
    ...
    {
        // Boxes setup
        float32_t qp_scale = raw_boxes_outputs[i]->qp_scale();
        float32_t qp_zp = raw_boxes_outputs[i]->qp_zp();
        ...
        // Keypoints setup
        float32_t qp_scale_kpts = raw_keypoints[i]->qp_scale();
        float32_t qp_zp_kpts = raw_keypoints[i]->qp_zp();
        ...
        xt::xarray<uint8_t> quantized_keypoints =
            xt::reshape_view(output_keypoints_quantized, {num_proposals_keypoints, 17, 3});
        ...
        for (uint j = 0; j < uint(num_proposals); j++)
        {
            ...
            HailoBBox bbox(decoded_box(j, 0) / network_dims[0], decoded_box(j, 1) / network_dims[1],
...
Triple get_boxes_scores_keypoints(std::vector<HailoTensorPtr> &tensors, int num_classes, int regression_length)
{
    ...
    for (uint i = 0; i < tensors.size(); i = i + 3)
    {
        ...
        auto dequantized_output_s = common::dequantize(common::get_xtensor(tensors[i+1]), tensors[i+1]->qp_scale(), tensors[i+1]->qp_zp());
        ...
        xt::view(scores, xt::range(view_index_scores, view_index_scores + num_proposals_scores), xt::all()) =
            xt::reshape_view(output_scores, {num_proposals_scores, num_classes});
...
std::vector<Decodings> yolov8pose_postprocess(std::vector<HailoTensorPtr> &tensors, std::vector<int> network_dims,
                                              std::vector<int> strides, int regression_length, int num_classes)
{
    std::vector<Decodings> decodings;
    if (tensors.size() == 0)
    {
...
std::pair<std::vector<KeyPt>, std::vector<PairPairs>> yolov8(HailoROIPtr roi)
{
    ...
    for (auto &dec : filtered_decodings)
    {
        ...
        std::pair<std::vector<KeyPt>, std::vector<PairPairs>> keypoints_and_pairs =
            process_single_decoding(dec, network_dims, 0.0f);
    ...
    std::pair<std::vector<KeyPt>, std::vector<PairPairs>> keypoints_and_pairs =
        filter_keypoints(filtered_decodings, network_dims);
