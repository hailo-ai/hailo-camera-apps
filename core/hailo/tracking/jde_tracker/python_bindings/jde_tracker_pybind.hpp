...
class JDETrackerWrapper
{
  public:
    ...
    static JDETrackerWrapper create(const float kalman_dist, const float iou_thr, const float init_iou_thr,
                                    ...
                                    float std_weight_position, float std_weight_pos_box, float std_weight_velocity,
                                    float std_weight_vel_box)
    {
        auto jde_tracker = std::make_unique<JDETracker>(
            JDETracker(kalman_dist, iou_thr, init_iou_thr, keep_tracked, keep_new, keep_lost, std_weight_position,
                       std_weight_pos_box, std_weight_velocity, std_weight_vel_box));
    ...
    {
        m_jde_tracker->set_kalman_distance(new_distance);
    }
    void set_iou_threshold(float new_iou_thr)
    {
        m_jde_tracker->set_iou_threshold(new_iou_thr);
    }
    void set_init_iou_threshold(float new_init_iou_thr)
    {
        m_jde_tracker->set_init_iou_threshold(new_init_iou_thr);
    }
    void set_keep_tracked_frames(int new_keep_tracked)
    {
        m_jde_tracker->set_keep_tracked_frames(new_keep_tracked);
    }
    void set_keep_new_frames(int new_keep_new)
    {
        m_jde_tracker->set_keep_new_frames(new_keep_new);
    }
    void set_keep_lost_frames(int new_keep_lost)
    {
        m_jde_tracker->set_keep_lost_frames(new_keep_lost);
    }

    // Getters for members access
    float get_kalman_distance()
    {
        return m_jde_tracker->get_kalman_distance();
    }
    float get_iou_threshold()
    {
        return m_jde_tracker->get_iou_threshold();
    }
    float get_init_iou_threshold()
    {
        return m_jde_tracker->get_init_iou_threshold();
    }
    float get_keep_tracked_frames()
    {
        return m_jde_tracker->get_keep_tracked_frames();
    }
    float get_keep_new_frames()
    {
        return m_jde_tracker->get_keep_new_frames();
    }
    float get_keep_lost_frames()
    {
        return m_jde_tracker->get_keep_lost_frames();
    }
     ...
     */
    py::list update(py::array_t<float, py::array::c_style | py::array::forcecast> input_detections,
                    bool report_unconfirmed)
    {
...
void JDETracker_api_initialize_python_module(py::module &m)
{
    py::class_<JDETrackerWrapper>(m, "JDETracker")
        .def(py::init(&JDETrackerWrapper::create), "kalman_dist"_a = 0.7, "iou_thr"_a = 0.8, "init_iou_thr"_a = 0.9,
             "keep_tracked"_a = 2, "keep_new"_a = 2, "keep_lost"_a = 2, "std_weight_position"_a = 0.01,
             "std_weight_pos_box"_a = 0.01, "std_weight_velocity"_a = 0.001, "std_weight_vel_box"_a = 0.001)
        ...
        .def("update", &JDETrackerWrapper::update, "input_detections"_a = nullptr, "report_unconfirmed"_a = false);
}
