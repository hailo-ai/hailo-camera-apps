 ...
 *
 */
TEST_CASE("JDE Tracker makes use of the lapjv.hpp library to solve linear assignment using Jonker-Volgenant algorithm.",
          "[lapjv]")
{
    ...
    SECTION("An NxN cost matrix should find a match for each object.")
    {
        ...
        int n = 3;
        std::vector<std::vector<float>> cost_c = {{0.00288438, 1.0, 1.0}, {1.0, 1.0, 0.31932773}, {1.0, 0.4, 1.0}};
    ...
    SECTION("An NxN-1 cost matrix should leave one object unmatched.")
    {
        ...
        int n = 3;
        std::vector<std::vector<float>> cost_c = {{0.00288438, 1.0}, {1.0, 1.0}, {1.0, 0.4}};
...
TEST_CASE("JDE Tracker can match new detections to tracked objects using linear assignment", "[jde_tracker_lapjv]")
{
    ...
    SECTION("An NxN cost matrix should find a match for each object.")
    {
        ...
        std::vector<std::vector<float>> cost_matrix = {{0.00288438, 1.0, 1.0}, {1.0, 1.0, 0.31932773}, {1.0, 0.4, 1.0}};
    ...
    SECTION("An NxN-1 cost matrix should leave one object unmatched.")
    {
        ...
        std::vector<int> tracked_matches;
        std::vector<int> detected_matches;
        std::vector<std::vector<float>> cost_matrix = {{0.00288438, 1.0}, {1.0, 1.0}, {1.0, 0.4}};
...
TEST_CASE("JDE Tracker can calculate iou distances between two sets of objects", "[jde_tracker_iou]")
{
    ...
    SECTION("Given two sets of length N, there should be an NxN cost matrix")
    {
        ...
        std::vector<std::vector<float>> tracked_objects = {
            {103.533936, 546.8082, 179.19987, 794.2363}, {50.0, 200.0, 100.0, 250.0}, {10.0, 100.0, 40.0, 150.0}};
        std::vector<std::vector<float>> detected_objects = {
            {103.56223, 546.8928, 179.32939, 794.3509}, {15.0, 105.0, 45.0, 155.0}, {55.0, 205.0, 105.0, 255.0}};

        // Expected cost matrix, recieved by adk tracker for same input objects
        std::vector<std::vector<float>> expected_cost_matrix = {
            {0.99711562, 0., 0.}, {0., 0., 0.68067227}, {0., 0.6, 0.}};
    ...
    SECTION("Given two sets of length N and N-1, there should be an NxN-1 cost matrix")
    {
        ...
        std::vector<std::vector<float>> tracked_objects = {
            {103.533936, 546.8082, 179.19987, 794.2363}, {50.0, 200.0, 100.0, 250.0}, {10.0, 100.0, 40.0, 150.0}};
        std::vector<std::vector<float>> detected_objects = {{103.56223, 546.8928, 179.32939, 794.3509},
                                                            {15.0, 105.0, 45.0, 155.0}};

        // Expected cost matrix, recieved by adk tracker for same input objects
        std::vector<std::vector<float>> expected_cost_matrix = {{0.99711562, 0.}, {0., 0.}, {0., 0.6}};
...
TEST_CASE("JDE Tracker can update its lst of tracked objects based on new detections", "[jde_tracker_update]")
{
    ...
    SECTION("Given a few frames of detections, some objects should be tracked.")
    {
        ...
        std::vector<std::vector<float>> expected_tlwhs2 = {
            {103.53394, 546.80823, 75.66593, 247.4281}, {50., 200., 50., 50.}, {10., 100., 30., 50.}};
        ...
        // The expected tracked objects, recieved by adk tracker for same input objects
        std::vector<std::vector<float>> expected_tlwhs3 = {{
                                                               103.75837161,
                                                               547.02259654,
                                                               75.54612335,
                                                               247.1281612,
                                                           },
                                                           {18.57142857, 108.57142857, 30., 50.},
                                                           {50., 200., 50., 50.}};
