...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_bitrate_debug, "bitrate", 0, "bitrate tracer");

#define gst_bitrate_tracer_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE(GstBitrateTracer, gst_bitrate_tracer, GST_TYPE_PERIODIC_TRACER, _do_init);
...
static void add_bytes(GstBitrateTracer *self, GstClockTime ts, GstPad *pad, guint64 bytes);
...
static gboolean do_print_bitrate(GstPeriodicTracer *tracer)
{
    ...
    while (g_hash_table_iter_next(&iter, &key, &value))
    {
        ...
        do_print_bitrate_event(BITRATE_EVENT_ID, pad_table->fullname, pad_table->bitrate);
...
static void do_destroy_hashtable_value(gpointer data)
{
...
static void do_destroy_hashtable_key(gpointer data)
{
...
static void add_bytes(GstBitrateTracer *self, GstClockTime ts, GstPad *pad, guint64 bytes)
{
    ...
    if (NULL == pad_frames)
    {
        ...
        g_hash_table_insert(self->bitrate_counters, gst_object_ref(pad), (gpointer)pad_frames);
    }
...
static void do_pad_push_buffer_pre(GstBitrateTracer *self, guint64 ts, GstPad *pad, GstBuffer *buffer)
{
...
static void do_pad_push_list_pre(GstBitrateTracer *self, GstClockTime ts, GstPad *pad, GstBufferList *list)
{
    guint idx;
    GstBuffer *buffer;
...
static void do_pad_pull_range_pre(GstBitrateTracer *self, GstClockTime ts, GstPad *pad, guint64 offset, guint size)
{
...
static void gst_bitrate_tracer_finalize(GObject *obj)
{
...
static void gst_bitrate_tracer_class_init(GstBitrateTracerClass *klass)
{
    ...
    tr_bitrate = gst_tracer_record_new(
        "bitrate.class", "pad", GST_TYPE_STRUCTURE,
        gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to", GST_TYPE_TRACER_VALUE_SCOPE,
                          GST_TRACER_VALUE_SCOPE_PAD, NULL),
        "bitrate", GST_TYPE_STRUCTURE,
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_UINT64, "description", G_TYPE_STRING, "Bitrate",
                          "flags", GST_TYPE_TRACER_VALUE_FLAGS, GST_TRACER_VALUE_FLAGS_AGGREGATED, "min", G_TYPE_UINT64,
                          G_GUINT64_CONSTANT(0), "max", G_TYPE_UINT64, G_MAXUINT64, NULL),
        NULL);
}

static void gst_bitrate_tracer_init(GstBitrateTracer *self)
{
    GstSharkTracer *tracer = GST_SHARK_TRACER(self);

    self->bitrate_counters =
        g_hash_table_new_full(g_direct_hash, g_direct_equal, do_destroy_hashtable_key, do_destroy_hashtable_value);

    gst_shark_tracer_register_hook(tracer, "pad-push-pre", G_CALLBACK(do_pad_push_buffer_pre));
    gst_shark_tracer_register_hook(tracer, "pad-push-list-pre", G_CALLBACK(do_pad_push_list_pre));
    gst_shark_tracer_register_hook(tracer, "pad-pull-range-pre", G_CALLBACK(do_pad_pull_range_pre));
}
...
static void create_metadata_event(GstPeriodicTracer *tracer)
{
    ...
    metadata_event = g_strdup_printf(bitrate_metadata_event, BITRATE_EVENT_ID, 0);
