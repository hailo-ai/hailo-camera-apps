 ...
 * @short_description: log scheduling time, which is the time between one incoming buffer and the next incoming buffer
 * in a sinkpad
 *
...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_scheduletime_debug, "scheduletime", 0, "scheduletime tracer");

G_DEFINE_TYPE_WITH_CODE(GstScheduletimeTracer, gst_scheduletime_tracer, GST_SHARK_TYPE_TRACER, _do_init);
...
static void do_push_buffer_list_pre(GstTracer *tracer, GstClockTime ts, GstPad *pad, GstBufferList *list);
static void gst_scheduletime_tracer_finalize(GObject *obj);

static void key_destroy(gpointer pad)
{
    gst_object_unref(pad);
}

static void schedule_pad_destroy(gpointer data)
{
    g_free(data);
}
...
static void sched_time_compute(GstTracer *tracer, guint64 ts, GstPad *pad)
{
    ...
    if (schedule_pad->previous_time != 0)
    {
        ...
        g_string_printf(time_string, "%" GST_TIME_FORMAT, GST_TIME_ARGS(time_diff));
...
static void gst_scheduletime_tracer_class_init(GstScheduletimeTracerClass *klass)
{
    ...
    tr_schedule =
        gst_tracer_record_new("scheduletime.class", "pad", GST_TYPE_STRUCTURE,
    ...
    metadata_event = g_strdup_printf(scheduling_metadata_event, SCHED_TIME_EVENT_ID, 0);
    add_metadata_event_struct(metadata_event);
    g_free(metadata_event);
}

static void gst_scheduletime_tracer_init(GstScheduletimeTracer *self)
{
    GstSharkTracer *tracer = GST_SHARK_TRACER(self);

    self->schedule_pads = g_hash_table_new_full(g_direct_hash, g_direct_equal, key_destroy, schedule_pad_destroy);

    gst_shark_tracer_register_hook(tracer, "pad-push-pre", G_CALLBACK(sched_time_compute));

    gst_shark_tracer_register_hook(tracer, "pad-push-list-pre", G_CALLBACK(do_push_buffer_list_pre));

    gst_shark_tracer_register_hook(tracer, "pad-pull-range-pre", G_CALLBACK(sched_time_compute));
}
