...
void gst_cpu_usage_compute(GstCPUUsage *cpu_usage)
{
    ...
    if (cpu_array_sel == 0)
    {
        ret = fscanf(fd, "%" S(CPU_NAME_MAX_SIZE) "s %d %d %d %d %d %d %d %d %d %d", cpu_name, &user[0], &nice[0],
                     &system[0], &idle[0], &iowait, &irq, &softirq, &steal, &quest, &quest_nice);
        for (cpu_id = 0; cpu_id < cpu_num; ++cpu_id)
        {
            ret = fscanf(fd, "%" S(CPU_NAME_MAX_SIZE) "s %d %d %d %d %d %d %d %d %d %d", cpu_name, &user[cpu_id],
                         &nice[cpu_id], &system[cpu_id], &idle[cpu_id], &iowait, &irq, &softirq, &steal, &quest,
                         &quest_nice);
        }
    ...
    else
    {
        ret = fscanf(fd, "%" S(CPU_NAME_MAX_SIZE) "s %d %d %d %d %d %d %d %d %d %d", cpu_name, &user_aux[0],
                     &nice_aux[0], &system_aux[0], &idle_aux[0], &iowait, &irq, &softirq, &steal, &quest, &quest_nice);
        for (cpu_id = 0; cpu_id < cpu_num; ++cpu_id)
        {
            ret = fscanf(fd, "%" S(CPU_NAME_MAX_SIZE) "s %d %d %d %d %d %d %d %d %d %d", cpu_name, &user_aux[cpu_id],
                         &nice_aux[cpu_id], &system_aux[cpu_id], &idle_aux[cpu_id], &iowait, &irq, &softirq, &steal,
                         &quest, &quest_nice);
        }
        /* Compute the utilization for each core */
        for (cpu_id = 0; cpu_id < cpu_num; ++cpu_id)
