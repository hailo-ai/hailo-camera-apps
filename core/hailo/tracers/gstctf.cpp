...
#define CTF_EVENT_WRITE_INT16(int16, mem) CTF_EVENT_WRITE(16, mem, int16)

#define CTF_EVENT_WRITE_INT32(int32, mem) CTF_EVENT_WRITE(32, mem, int32)

#define CTF_EVENT_WRITE_INT64(int64, mem) CTF_EVENT_WRITE(64, mem, int64)
...
static gboolean event_exceeds_mem_size(const gsize size)
{
    gboolean ret;
...
static GstCtfDescriptor *ctf_create_struct(void)
{
    gchar UUID[] = {
        static_cast<gchar>(0xd1), static_cast<gchar>(0x8e), static_cast<gchar>(0x63), static_cast<gchar>(0x74),
        static_cast<gchar>(0x35), static_cast<gchar>(0xa1), static_cast<gchar>(0xcd), static_cast<gchar>(0x42),
        static_cast<gchar>(0x8e), static_cast<gchar>(0x70), static_cast<gchar>(0xa9), static_cast<gchar>(0xcf),
...
static void generate_datastream_header(void)
{
    ...
    if (FALSE == ctf_descriptor->tcp_output_disable)
    {
        ...
        g_output_stream_write(ctf_descriptor->output_stream, ctf_descriptor->mem, event_size + TCP_HEADER_SIZE, NULL,
                              &error);
    }
...
static void uuid_to_uuidstring(gchar *uuid_string, guint8 *uuid)
{
...
static void generate_metadata(gint major, gint minor, gint byte_order)
{
    gint str_len;
    GError *error;
    guint8 *event_mem;
    ...
    str_len =
        g_snprintf((gchar *)event_mem, CTF_MEM_SIZE, metadata_fmt, major, minor, uuid_string, byte_order ? "le" : "be");
    ...
    if (FALSE == ctf_descriptor->tcp_output_disable)
    {
        ...
        g_output_stream_write(ctf_descriptor->output_stream, ctf_descriptor->mem, str_len + TCP_HEADER_SIZE, NULL,
                              &error);
    }
    g_mutex_unlock(&ctf_descriptor->mutex);
}
...
static void tcp_parser_handler(gchar *line)
{
    ...
    if (*line_end == ':')
    {
        ...
        *line_end = '\0';
        ...
        ctf_descriptor->port_number = g_ascii_strtoull(port_name, &port_name_end, 10);

        /* Verify if the convertion of the string works */
        if ('\0' != *port_name_end || '-' == port_name[0])
        {
            ctf_descriptor->port_number = SOCKET_PORT;
            GST_ERROR("Invalid port number \"%s\", using the default value: %d", port_name,
...
static void ctf_process_env_var(void)
{
    ...
    if (NULL != env_loc_value)
    {
        ...
        parser_register_callbacks(parser, parser_handler_desc_list,
                                  sizeof(parser_handler_desc_list) / sizeof(parser_handler_desc), file_parser_handler);
    ...
    {
        ctf_descriptor->file_buf_size = g_ascii_strtoull(env_file_buf_value, &env_file_buf_value_end, 10);
        ...
        {
            GST_ERROR("Invalid buffer size \"%s\", using default system value", env_file_buf_value);
        }
    }
...
static int create_ctf_path(gchar *dir_name)
{
    int ret = 0;
    ...
    if (!g_file_test(dir_name, G_FILE_TEST_EXISTS))
    {
        ...
        if (ret == 0)
        {
            GST_INFO("Directory %s did not exist and was created sucessfully.", dir_name);
        }
        else
        {
...
static void ctf_file_init(void)
{
    ...
    if (TRUE != ctf_descriptor->file_output_disable)
    {
        ...
        {
            datastream_file = g_strjoin(G_DIR_SEPARATOR_S, ctf_descriptor->dir_name, "datastream", NULL);
            metadata_file = g_strjoin(G_DIR_SEPARATOR_S, ctf_descriptor->dir_name, "metadata", NULL);
            ...
            if (ctf_descriptor->change_file_buf_size)
            {
                ...
                {
                    setvbuf(ctf_descriptor->metadata, NULL, _IOFBF, ctf_descriptor->file_buf_size);
                    setvbuf(ctf_descriptor->datastream, NULL, _IOFBF, ctf_descriptor->file_buf_size);
                }
            }
...
static void ctf_tcp_init(void)
{
    ...
    error = NULL;
    socket_connection = g_socket_client_connect_to_host(socket_client, ctf_descriptor->host_name,
                                                        ctf_descriptor->port_number, NULL, &error);
...
void do_print_interlatency_event(event_id id, gchar *originpad, gchar *destinationpad, guint64 time)
{
    GError *error;
    guint8 *mem;
    guint8 *event_mem;
    ...
    event_size = strlen(originpad) + 1 + strlen(destinationpad) + 1 + sizeof(guint64) + CTF_HEADER_SIZE;
...
void do_print_queue_level_event(event_id id, const gchar *elementname, guint32 bytes, guint32 max_bytes,
                                guint32 buffers, guint32 max_buffers, guint64 time, guint64 max_time)
{
    GError *error;
    guint8 *mem;
    guint8 *event_mem;
    ...
    event_size = strlen(elementname) + 1 + 4 * sizeof(guint32) + 2 * sizeof(guint64) + CTF_HEADER_SIZE;
...
void do_print_buffer_event(event_id id, const gchar *pad, GstClockTime pts, GstClockTime dts, GstClockTime duration,
                           guint64 offset, guint64 offset_end, guint64 size, GstBufferFlags flags, guint32 refcount)
{
    GError *error;
    guint8 *mem;
    guint8 *event_mem;
    ...
    event_size = strlen(pad) + 1 + 6 * sizeof(guint64) + 2 * sizeof(guint32) + CTF_HEADER_SIZE;
