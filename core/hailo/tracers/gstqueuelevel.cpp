...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_queue_level_debug, "queuelevel", 0, "queuelevel tracer");

G_DEFINE_TYPE_WITH_CODE(GstQueueLevelTracer, gst_queue_level_tracer, GST_SHARK_TYPE_TRACER, _do_init);

static void do_queue_level(GstTracer *tracer, guint64 ts, GstPad *pad);
static void do_queue_level_list(GstTracer *tracer, guint64 ts, GstPad *pad, GstBufferList *list);
...
static void do_queue_level(GstTracer *self, guint64 ts, GstPad *pad)
{
    ...
    g_object_get(element, "current-level-bytes", &size_bytes, "current-level-buffers", &size_buffers,
                 "current-level-time", &size_time, "max-size-bytes", &max_size_bytes, "max-size-buffers",
                 &max_size_buffers, "max-size-time", &max_size_time, NULL);

    size_time_string = g_strdup_printf("%" GST_TIME_FORMAT, GST_TIME_ARGS(size_time));

    max_size_time_string = g_strdup_printf("%" GST_TIME_FORMAT, GST_TIME_ARGS(max_size_time));

    gst_tracer_record_log(tr_qlevel, element_name, size_bytes, max_size_bytes, size_buffers, max_size_buffers,
                          size_time_string, max_size_time_string);

    g_free(size_time_string);
    g_free(max_size_time_string);

    do_print_queue_level_event(QUEUE_LEVEL_EVENT_ID, element_name, size_bytes, max_size_bytes, size_buffers,
                               max_size_buffers, size_time, max_size_time);
...
/* tracer class */
static void gst_queue_level_tracer_class_init(GstQueueLevelTracerClass *klass)
{
    gchar *metadata_event;

    tr_qlevel =
        gst_tracer_record_new("queuelevel.class", "queue", GST_TYPE_STRUCTURE,
                              ...
                              "size_bytes", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_UINT, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              "max_size_bytes", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_UINT, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              "size_buffers", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_UINT, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              "max_size_buffers", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_UINT, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              "size_time", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              "max_size_time", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              NULL);

    metadata_event = g_strdup_printf(queue_level_metadata_event, QUEUE_LEVEL_EVENT_ID, 0);
    add_metadata_event_struct(metadata_event);
    g_free(metadata_event);
}

static void gst_queue_level_tracer_init(GstQueueLevelTracer *self)
{
    GstSharkTracer *tracer = GST_SHARK_TRACER(self);

    gst_shark_tracer_register_hook(tracer, "pad-push-pre", G_CALLBACK(do_queue_level));

    gst_shark_tracer_register_hook(tracer, "pad-push-list-pre", G_CALLBACK(do_queue_level_list));

    gst_shark_tracer_register_hook(tracer, "pad-pull-range-pre", G_CALLBACK(do_queue_level));
}
