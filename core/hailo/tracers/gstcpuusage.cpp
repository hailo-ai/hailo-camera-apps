...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_cpu_usage_debug, "cpuusage", 0, "cpuusage tracer");

G_DEFINE_TYPE_WITH_CODE(GstCPUUsageTracer, gst_cpu_usage_tracer, GST_TYPE_PERIODIC_TRACER, _do_init);
...
static void cpuusage_dummy_bin_add_post(GObject *obj, GstClockTime ts, GstBin *bin, GstElement *element,
                                        gboolean result);
...
static void create_metadata_event(GstPeriodicTracer *tracer)
{
    ...
    for (msg_id = 0; msg_id < cpu_num; ++msg_id)
    {
         ...
         */
        number_of_bytes = g_snprintf(mem, mem_size, floating_point_event_field,
                                     (unsigned long)(sizeof(gfloat) * CHAR_BIT - FLT_MANT_DIG), FLT_MANT_DIG, msg_id);
...
static void gst_cpu_usage_tracer_class_init(GstCPUUsageTracerClass *klass)
{
    ...
    tr_cpuusage = gst_tracer_record_new(
        "cpuusage.class", "number", GST_TYPE_STRUCTURE,
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_UINT, "description", G_TYPE_STRING, "Core number",
                          "flags", GST_TYPE_TRACER_VALUE_FLAGS, GST_TRACER_VALUE_FLAGS_AGGREGATED, "min", G_TYPE_UINT,
                          0, "max", G_TYPE_UINT, CPU_NUM_MAX, NULL),
        "load", GST_TYPE_STRUCTURE,
...
static void gst_cpu_usage_tracer_init(GstCPUUsageTracer *self)
{
    GstCPUUsage *cpu_usage;
    ...
    gst_tracing_register_hook(GST_TRACER(self), "bin-add-post", G_CALLBACK(cpuusage_dummy_bin_add_post));
}
