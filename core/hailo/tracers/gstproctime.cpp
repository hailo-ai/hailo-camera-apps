...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_proc_time_debug, "proctime", 0, "proctime tracer");

G_DEFINE_TYPE_WITH_CODE(GstProcTimeTracer, gst_proc_time_tracer, GST_SHARK_TYPE_TRACER, _do_init);
...
static void do_push_buffer_pre(GstTracer *self, guint64 ts, GstPad *pad)
{
    ...
    should_log = gst_proctime_proc_time(proc_time, &time, pad_peer, pad, ts, should_calculate);
...
static void do_element_new(GObject *self, GstClockTime ts, GstElement *element)
{
    GstProcTimeTracer *proc_time_tracer;
    GstProcTime *proc_time;
...
static void gst_proc_time_tracer_finalize(GObject *obj)
{
...
static void gst_proc_time_tracer_class_init(GstProcTimeTracerClass *klass)
{
    ...
    tr_proc_time =
        gst_tracer_record_new("proctime.class", "element", GST_TYPE_STRUCTURE,
    ...
    metadata_event = g_strdup_printf(proc_time_metadata_event, PROCTIME_EVENT_ID, 0);
    add_metadata_event_struct(metadata_event);
    g_free(metadata_event);
}

static void gst_proc_time_tracer_init(GstProcTimeTracer *self)
{
    ...
    gst_tracing_register_hook(tracer, "element-new", G_CALLBACK(do_element_new));
}
