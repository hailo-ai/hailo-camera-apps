...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_buffer_drop_debug, "bufferdrop", 0, "bufferdrop tracer");
...
static void do_buffer_drop_list(GstTracer *tracer, guint64 ts, GstPad *pad, GstBufferList *list);
...
static void do_buffer_drop(GstTracer *self, guint64 ts, GstPad *pad)
{
    GstElement *element;
    ...
    max_size_time_string = g_strdup_printf("%" GST_TIME_FORMAT, GST_TIME_ARGS(frames_dropped));
...
/* tracer class */
static void gst_buffer_drop_tracer_class_init(GstBufferDropTracerClass *klass)
{
    tr_qlevel =
        gst_tracer_record_new("bufferdrop.class", "name", GST_TYPE_STRUCTURE,
                              ...
                              "buffers_drop", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_UINT, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_ELEMENT, NULL),
                              NULL);
}

static void gst_buffer_drop_tracer_init(GstBufferDropTracer *self)
{
    GstSharkTracer *tracer = GST_SHARK_TRACER(self);

    gst_shark_tracer_register_hook(tracer, "pad-push-pre", G_CALLBACK(do_buffer_drop));

    gst_shark_tracer_register_hook(tracer, "pad-push-list-pre", G_CALLBACK(do_buffer_drop_list));

    gst_shark_tracer_register_hook(tracer, "pad-pull-range-pre", G_CALLBACK(do_buffer_drop));
}
