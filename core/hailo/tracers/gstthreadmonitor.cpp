...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_thread_monitor_debug, "threadmonitor", 0, "threadmonitor tracer");

G_DEFINE_TYPE_WITH_CODE(GstThreadMonitorTracer, gst_thread_monitor_tracer, GST_TYPE_PERIODIC_TRACER, _do_init);

static GstTracerRecord *tr_threadmonitor;

static void threadmonitor_dummy_bin_add_post(GObject *obj, GstClockTime ts, GstBin *bin, GstElement *element,
                                             gboolean result);
...
static void gst_thread_monitor_tracer_class_init(GstThreadMonitorTracerClass *klass)
{
    ...
    tr_threadmonitor = gst_tracer_record_new(
        "threadmonitor.class", "name", GST_TYPE_STRUCTURE,
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_STRING, "description", G_TYPE_STRING, "thread name",
                          "flags", GST_TYPE_TRACER_VALUE_FLAGS, GST_TRACER_VALUE_FLAGS_AGGREGATED, NULL),
        "cpu_usage", GST_TYPE_STRUCTURE,
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_DOUBLE, "description", G_TYPE_STRING,
                          "CPU usage of thread percentage [%]", "flags", GST_TYPE_TRACER_VALUE_FLAGS,
                          GST_TRACER_VALUE_FLAGS_AGGREGATED, "min", G_TYPE_DOUBLE, 0.0f, "max", G_TYPE_DOUBLE, 100.0f,
                          NULL),

        "memory_usage", GST_TYPE_STRUCTURE,
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_DOUBLE, "description", G_TYPE_STRING,
                          "Memory usage of thread percentage [%]", "flags", GST_TYPE_TRACER_VALUE_FLAGS,
                          GST_TRACER_VALUE_FLAGS_AGGREGATED, "min", G_TYPE_DOUBLE, 0.0f, "max", G_TYPE_DOUBLE, 100.0f,
                          NULL),
        NULL);
}

static void gst_thread_monitor_tracer_init(GstThreadMonitorTracer *self)
{
    ...
    gst_tracing_register_hook(GST_TRACER(self), "bin-add-post", G_CALLBACK(threadmonitor_dummy_bin_add_post));
}
