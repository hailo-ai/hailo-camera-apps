...
std::vector<std::string> blacklist_thread_names = {"gst-launch-1.0", "qtdemux0:sink", "gmain", "typefind:sink",
                                                   "pool-gst-launch"};
...
void gst_thread_monitor_init(GstThreadMonitor *thread_monitor)
{
    ...
    colums_command =
        g_strdup_printf("top -b -H-p %d -n 1 | grep PID | tr -s ' ' | sed -e 's/\x1b\[[0-9;]*m//g'", getpid());
...
void gst_thread_monitor_compute(GstTracerRecord *tr_threadmonitor, GstThreadMonitor *thread_monitor,
                                gchar **thread_name, gchar **thread_cpu_usage, gchar **thread_memory_usage)
{
    FILE *fp;
    gchar *command;
    char **tokens;
    ...
    command = g_strdup_printf("top -H -b -p %d -n 1 | sed -n '/PID/,/^$/p' | tail -n +2 | tr -s ' ' | sed -e "
                              "'s/\x1b\[[0-9;]*m//g' | awk '{print $%d,$%d,$%d}'",
                              getpid(), thread_monitor->thread_name_loc, thread_monitor->thread_cpu_usage_loc,
                              thread_monitor->thread_memory_usage_loc);
