...
void gst_dot_pipeline_to_file(GstBin *bin, GstDebugGraphDetails flags)
{
    gchar *trace_dir;
    ...
    full_trace_dir = g_strdup_printf("%s" G_DIR_SEPARATOR_S "graphic", trace_dir);

    if (!g_file_test(full_trace_dir, G_FILE_TEST_EXISTS))
    {
        if (g_mkdir(full_trace_dir, 0775) == 0)
        {
            GST_INFO("Directory %s did not exist and was created sucessfully.", full_trace_dir);
        }
        else
        {
    ...
    {
        GST_INFO("Directory %s already exists in the current path.", full_trace_dir);
    }

    strftime(time_suffix, MAX_SUFFIX_LEN, "%F_%T", localtime(&now));

    full_file_name =
        g_strdup_printf("%s" G_DIR_SEPARATOR_S "%s_%s.dot", full_trace_dir, GST_OBJECT_NAME(bin), time_suffix);
...
static gpointer gst_dot_render_thread(gpointer data)
{
