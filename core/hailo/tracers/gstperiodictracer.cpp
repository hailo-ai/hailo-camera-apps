...
static void element_change_state_post(GstTracer *self, guint64 ts, GstElement *element, GstStateChange transition,
...
G_DEFINE_TYPE_WITH_PRIVATE(GstPeriodicTracer, gst_periodic_tracer, GST_SHARK_TYPE_TRACER);

static void gst_periodic_tracer_class_init(GstPeriodicTracerClass *klass)
{
    GST_DEBUG_CATEGORY_INIT(gst_periodic_debug, "periodictracer", 0, "base periodic tracer");
...
static void gst_periodic_tracer_init(GstPeriodicTracer *self)
{
...
static void element_change_state_post(GstTracer *tracer, guint64 ts, GstElement *element, GstStateChange transition,
                                      GstStateChangeReturn result)
{
    ...
    if (transition == GST_STATE_CHANGE_PAUSED_TO_PLAYING && result == GST_STATE_CHANGE_SUCCESS)
    {
        GST_DEBUG_OBJECT(self, "Pipeline %s changed to playing", GST_OBJECT_NAME(element));
    ...
    {
        GST_DEBUG_OBJECT(self, "Pipeline %s changed to paused", GST_OBJECT_NAME(element));
...
static void install_callback(GstPeriodicTracer *self)
{
    ...
    {
        GST_INFO_OBJECT(self, "First pipeline started running, starting profiling");

        priv->period = set_period(self);
        priv->callback_id = g_timeout_add_seconds(priv->period, (GSourceFunc)callback_internal, (gpointer)self);
    }
...
static gint set_period(GstPeriodicTracer *self)
{
    ...
    if (NULL == list)
    {
        GST_INFO_OBJECT(self, "No period specifying, using default of %d", DEFAULT_TIMEOUT_INTERVAL);
        period = DEFAULT_TIMEOUT_INTERVAL;
    }
    else
    {
        GST_INFO_OBJECT(self, "Attempting to parse provided period \"%s\"", (gchar *)list->data);
