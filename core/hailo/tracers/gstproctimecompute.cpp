...
static void gst_proctime_add_in_list(GstProcTime *proc_time, GstPad *sink_pad, GstPad *src_pad);
...
void gst_proctime_free(GstProcTime *self)
{
    ...
    for (elem_idx = 0; elem_idx < elem_num; ++elem_idx)
    {
        ...
        if (element->src_pad == src_pad)
        {
            ...
            else
            {
                ...
                GST_WARNING_OBJECT(element->src_pad, "Timestamps mismatch, this should not happen");
                found = FALSE;
                goto exit;
            }
            found = TRUE;
