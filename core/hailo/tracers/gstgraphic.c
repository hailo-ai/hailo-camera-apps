...
G_DEFINE_TYPE_WITH_CODE(GstGraphicTracer, gst_graphic_tracer, GST_SHARK_TYPE_TRACER, _do_init);

static void do_element_change_state_post(GstGraphicTracer *self, guint64 ts, GstElement *element,
                                         ...
                                         GstStateChange transition, GstStateChangeReturn result)
{
    ...
    if (GST_IS_PIPELINE(element) && (transition == GST_STATE_CHANGE_PAUSED_TO_PLAYING))
    {
        /* Logging the change of state in which the pipeline graphic is being done */
        GST_CAT_TRACE(GST_CAT_STATES, "%" GST_TIME_FORMAT ", element=%" GST_PTR_FORMAT ", change=%d, res=%d",
...
static void gst_graphic_tracer_class_init(GstGraphicTracerClass *klass)
{
...
static void gst_graphic_tracer_init(GstGraphicTracer *self)
{
