...
#define _do_init GST_DEBUG_CATEGORY_INIT(gst_framerate_debug, "framerate", 0, "framerate tracer");

G_DEFINE_TYPE_WITH_CODE(GstFramerateTracer, gst_framerate_tracer, GST_TYPE_PERIODIC_TRACER, _do_init);
...
static void consider_frames(GstFramerateTracer *self, GstPad *pad, guint amount);
static void destroy_hashtable_value(gpointer data);
static void pad_push_buffer_pre(GstFramerateTracer *self, guint64 ts, GstPad *pad, GstBuffer *buffer);
static void pad_push_list_pre(GstFramerateTracer *self, GstClockTime ts, GstPad *pad, GstBufferList *list);
static void pad_pull_range_pre(GstFramerateTracer *self, GstClockTime ts, GstPad *pad, guint64 offset, guint size);
...
static void gst_framerate_tracer_class_init(GstFramerateTracerClass *klass)
{
    ...
    tr_framerate = gst_tracer_record_new(
        "framerate.class", "pad", GST_TYPE_STRUCTURE,
        ...
        gst_structure_new("value", "type", G_TYPE_GTYPE, G_TYPE_UINT, "description", G_TYPE_STRING, "Frames per second",
                          "flags", GST_TYPE_TRACER_VALUE_FLAGS, GST_TRACER_VALUE_FLAGS_AGGREGATED, "min", G_TYPE_UINT,
                          0, "max", G_TYPE_UINT, G_MAXUINT, NULL),
        NULL);
}

static void gst_framerate_tracer_init(GstFramerateTracer *self)
{
    GstSharkTracer *stracer = GST_SHARK_TRACER(self);

    self->frame_counters =
        g_hash_table_new_full(g_direct_hash, g_direct_equal, gst_object_unref, destroy_hashtable_value);

    gst_shark_tracer_register_hook(stracer, "pad-push-pre", G_CALLBACK(pad_push_buffer_pre));
    gst_shark_tracer_register_hook(stracer, "pad-push-list-pre", G_CALLBACK(pad_push_list_pre));
    gst_shark_tracer_register_hook(stracer, "pad-pull-range-pre", G_CALLBACK(pad_pull_range_pre));
}
...
static gboolean print_framerate(GstPeriodicTracer *tracer)
{
    ...
    while (g_hash_table_iter_next(&iter, &key, &value))
    {
        ...
        gst_tracer_record_log(tr_framerate, pad_table->fullname, pad_table->counter);
        do_print_framerate_event(FPS_EVENT_ID, pad_table->fullname, pad_table->counter);
        pad_table->counter = 0;
    }
...
static void consider_frames(GstFramerateTracer *self, GstPad *pad, guint amount)
{
    ...
    pad_frames = (GstFramerateHash *)g_hash_table_lookup(self->frame_counters, pad);
    ...
    else
    {
        ...
        GST_OBJECT_LOCK(self);
        g_hash_table_insert(self->frame_counters, gst_object_ref(pad), pad_frames);
        GST_OBJECT_UNLOCK(self);
