...
G_DEFINE_TYPE_WITH_CODE(GstInterLatencyTracer, gst_interlatency_tracer, GST_SHARK_TYPE_TRACER, _do_init);
...
static void log_latency(GstInterLatencyTracer *interlatency_tracer, const GstStructure *data, GstPad *sink_pad,
                        guint64 sink_ts)
{
    GstPad *src_pad = NULL;
    guint64 src_ts;
    ...
    gst_structure_id_get(data, latency_probe_pad, GST_TYPE_PAD, &src_pad, latency_probe_ts, G_TYPE_UINT64, &src_ts,
                         NULL);
...
static void send_latency_probe(GstElement *parent, GstPad *pad, guint64 ts)
{
    ...
    {
        GstEvent *latency_probe = gst_event_new_custom(
            GST_EVENT_CUSTOM_DOWNSTREAM, gst_structure_new_id(latency_probe_id, latency_probe_pad, GST_TYPE_PAD, pad,
                                                              latency_probe_ts, G_TYPE_UINT64, ts, NULL));
        gst_pad_push_event(pad, latency_probe);
    }
}

static void calculate_latency(GstInterLatencyTracer *interlatency_tracer, GstElement *parent, GstPad *pad, guint64 ts)
{
...
static void do_push_buffer_pre(GstTracer *self, guint64 ts, GstPad *pad)
{
    ...
    if (peer_parent && GST_OBJECT_FLAG_IS_SET(peer_parent, GST_ELEMENT_FLAG_SINK))
        calculate_latency(interlatency_tracer, peer_parent, peer_pad, ts);
}

static void do_pull_range_pre(GstTracer *self, guint64 ts, GstPad *pad)
{
    GstInterLatencyTracer *interlatency_tracer;
    ...
    if (parent_peer && GST_OBJECT_FLAG_IS_SET(parent_peer, GST_ELEMENT_FLAG_SOURCE))
        send_latency_probe(parent_peer, peer_pad, ts);
    else
        calculate_latency(interlatency_tracer, parent_peer, peer_pad, ts);
}

static void do_pull_range_post(GstTracer *self, guint64 ts, GstPad *pad)
{
    GstInterLatencyTracer *interlatency_tracer;
    GstElement *parent = get_real_pad_parent(pad);
...
static void do_push_event_pre(GstTracer *self, guint64 ts, GstPad *pad, GstEvent *ev)
{
    GstPad *peer_pad = GST_PAD_PEER(pad);
    GstElement *parent = get_real_pad_parent(pad);
    GstElement *parent_peer = get_real_pad_parent(peer_pad);

    if (parent_peer && (!GST_IS_BIN(parent_peer)) && !GST_OBJECT_FLAG_IS_SET(parent, GST_ELEMENT_FLAG_SOURCE))
    {
        if (GST_EVENT_TYPE(ev) == GST_EVENT_CUSTOM_DOWNSTREAM)
        {
            ...
            if (gst_structure_get_name_id(data) == latency_probe_id)
            {
                 ...
                 * has been processed */
                g_object_set_qdata_full((GObject *)pad, latency_probe_id, gst_event_ref(ev),
                                        (GDestroyNotify)gst_event_unref);
                if (GST_OBJECT_FLAG_IS_SET(parent_peer, GST_ELEMENT_FLAG_SINK))
                    g_object_set_qdata_full((GObject *)peer_pad, latency_probe_id, gst_event_ref(ev),
                                            (GDestroyNotify)gst_event_unref);
            }
        }
    }
...
static void gst_interlatency_tracer_class_init(GstInterLatencyTracerClass *klass)
{
    GObjectClass *oclass;
    ...
    tr_interlatency =
        gst_tracer_record_new("interlatency.class", "from_pad", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_PAD, NULL),
                              "to_pad", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_PAD, NULL),
                              "time", GST_TYPE_STRUCTURE,
                              gst_structure_new("scope", "type", G_TYPE_GTYPE, G_TYPE_STRING, "related-to",
                                                GST_TYPE_TRACER_VALUE_SCOPE, GST_TRACER_VALUE_SCOPE_PROCESS, NULL),
                              NULL);

    oclass->dispose = gst_interlatency_tracer_dispose;

    metadata_event = g_strdup_printf(interlatency_metadata_event, INTERLATENCY_EVENT_ID, 0);
    add_metadata_event_struct(metadata_event);
    g_free(metadata_event);
}

static void gst_interlatency_tracer_init(GstInterLatencyTracer *self)
{
    ...
    gst_tracing_register_hook(tracer, "pad-push-list-pre", G_CALLBACK(do_push_buffer_pre));
